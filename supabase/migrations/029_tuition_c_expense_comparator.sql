-- ============================================================================
-- Migration 029: Tuition-C — Stage 1 expense comparator
--
-- v3.8.7 (Tuition-C). Stage 1 KPIs (Net Education Program Ratio,
-- Breakeven Enrollment) measure scenarios against an expense
-- comparator the system fetches from the most recent locked Budget
-- OR the user enters manually.
--
-- The cart-and-horse problem: Tuition Planning happens approximately
-- three months before the same-AYE Preliminary Budget per cascade
-- rule §7.5. Same-AYE Preliminary is rarely available at Tuition
-- Planning time. Default comparator is the most recent locked Budget
-- for the school (typically the prior AYE's Final); the user can
-- switch to manual entry.
--
-- Schema scope:
--   1. Three new columns on tuition_worksheet_scenarios + snapshots:
--      expense_comparator_mode text NOT NULL DEFAULT 'locked_budget'
--        with CHECK ('locked_budget' | 'manual')
--      expense_comparator_amount numeric(12,2) NULL
--      expense_comparator_source_label text NULL
--   2. New SECURITY DEFINER RPC get_latest_locked_budget_for_school
--      returning the most-recent locked Budget snapshot's KPI
--      total_expenses, AYE label, and stage type.
--   3. lock_tuition_scenario refreshed via CREATE OR REPLACE: snapshot
--      INSERT extended to write all three new columns.
--   4. create_tuition_scenario_from_snapshot refreshed via CREATE OR
--      REPLACE: copies all three new columns forward when seeding
--      Stage 2 from a Stage 1 snapshot.
--   5. compute_tuition_scenario_kpis NOT touched — Stage 1 KPIs
--      continue computing client-side per the v3.8.3 architectural
--      decision; server-side hoist remains queued for Tuition-C+
--      when the RPC starts returning a richer KPI bundle.
--
-- Architecture references: §7.3 (extended with "Stage 1 expense
-- comparator" subsection in v3.8.7), §7.5 (cart-and-horse cascade
-- rationale), §7.6 (Module-to-Budget mapping).
-- ============================================================================


-- ---- 1. Schema additions on scenarios -----------------------------------
--
-- expense_comparator_mode is NOT NULL DEFAULT 'locked_budget' so
-- existing rows backfill cleanly without per-row UPDATE. The CHECK
-- constraint enforces the two-value vocabulary at the database level;
-- adding future modes (e.g., 'specific_budget' for picking a
-- particular locked Budget by id) requires a follow-up migration to
-- relax the constraint.

alter table tuition_worksheet_scenarios
  add column expense_comparator_mode         text not null default 'locked_budget'
    check (expense_comparator_mode in ('locked_budget', 'manual')),
  add column expense_comparator_amount       numeric(12,2),
  add column expense_comparator_source_label text;


-- ---- 2. Schema additions on snapshots -----------------------------------
--
-- Mirror the three columns on tuition_worksheet_snapshots so the
-- captured-by-value pattern preserves the comparator state at lock
-- time (architecture §5.1 binding rule). The mode column has the
-- same CHECK constraint; amount and source_label are nullable.
...
-- ---- 3. get_latest_locked_budget_for_school RPC -------------------------
--
-- Returns the most-recent locked Budget snapshot's data for use as
-- the Tuition Stage 1 expense comparator. Single row or no rows.
--
-- Single-tenant note: Agora has no school_id column anywhere in the
-- schema (single-tenant by design per architecture §3). The
-- p_school_id parameter is preserved for forward-compatible naming —
-- when multi-tenancy lands, this function becomes school-scoped via
-- a WHERE clause + index on the corresponding column. Today the
-- parameter is ignored; the function returns the latest locked
-- Budget for the only school in the system.
--
-- Ordering: academic_years.start_date DESC (later AYE wins) then
-- locked_at DESC (later lock wins among same-AYE). This matches user
-- intent — the staleness-aware comparator wants the latest AYE's
-- Budget, even if an older AYE was re-locked more recently.
--
-- Source of total_expenses: budget_snapshots.kpi_total_expenses,
-- captured by value at lock time per Migration 011/012. We do NOT
-- recompute from live budget_stage_lines — the locked snapshot is
-- the official record per §5.1 binding rule.

create or replace function get_latest_locked_budget_for_school(p_school_id uuid)
returns table (
  scenario_id    uuid,
  aye            text,
  stage_type     text,
...
as $$
  ...
  -- Single-tenant: p_school_id is intentionally unused. Documented
  -- in the function header.
  order by ay.start_date desc, bs.locked_at desc
  limit 1
$$;

grant execute on function get_latest_locked_budget_for_school(uuid) to authenticated;


-- ---- 4. lock_tuition_scenario — pass new columns through ----------------
--
-- Snapshot INSERT extended with the three new comparator columns.
-- Return type (uuid) and validation logic unchanged → CREATE OR
-- REPLACE works.
...
-- ---- 5. create_tuition_scenario_from_snapshot — seed copy ---------------
--
-- Stage 2 (Tuition Audit) seed extended to copy the three new
-- comparator columns forward from the locked Stage 1 snapshot. The
-- comparator state seeded here is informational at Stage 2 — Stage 2
-- KPIs against actuals would naturally use the same comparator, but
-- the user can switch modes or update the value as audit needs change.
