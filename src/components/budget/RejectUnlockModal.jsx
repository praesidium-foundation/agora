...
// Modal driving the reject branch of reject_budget_stage_unlock.
//
// Same RPC as withdraw — H1's function auto-detects the path by
// comparing the caller's auth.uid() to scenario.unlock_requested_by.
// Caller is NOT the requester → recorded as 'unlock_rejected'.
// Caller IS the requester → recorded as 'unlock_withdrawn'.
//
// The two modals are deliberately distinct UI even though they share
// the RPC: rejection is a destructive governance action (denying
// someone else's request); withdraw is housekeeping (cancelling your
// own). Different copy, different button styling.
//
// Props:
...
//
// Requester display name is resolved internally from user_profiles
// (same pattern as LockedBanner / ApproveUnlockModal).
