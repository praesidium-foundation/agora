...
// Modal driving the withdraw branch of reject_tuition_scenario_unlock.
//
// Same RPC as reject — Migration 025's function detects the path by
// comparing the caller to scenario.unlock_requested_by. Caller IS the
// requester → recorded as 'unlock_withdrawn' with the user's reason.
//
// Withdraw is housekeeping (cancelling your own request); rejection
// is governance (denying someone else's). Different copy, different
// button styling. Mirrors src/components/budget/WithdrawUnlockModal.jsx.
    ...
    >
      ...
      >
        ...
        <div className="px-6 py-5 overflow-y-auto">
          ...
          <FieldLabel htmlFor="tuition-withdraw-reason">
            Why are you withdrawing? (required)
          </FieldLabel>
          <textarea
            id="tuition-withdraw-reason"
            value={reason}
            onChange={(e) => setReason(e.target.value)}
            rows={3}
            placeholder="A short note for the audit log — e.g. waiting on Tuition Committee input, no longer needed, request was premature."
