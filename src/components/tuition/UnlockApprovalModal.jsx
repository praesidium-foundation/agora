...
// Modal that drives the approve_tuition_scenario_unlock RPC.
//
// v3.7 two-identity model. Approval_1 is always already populated by
// the time this modal mounts — the requester's submission counts as
// approval_1 (Migration 025's request_tuition_scenario_unlock writes
// both request fields and approval_1 fields atomically). So this
// modal only ever handles approval_2 + the state transition.
//
// Mirrors src/components/budget/ApproveUnlockModal.jsx in shape.
//
...
// layer (canApproveUnlock returns {ok: false, reason: 'is_initiator'}
// for the requester). The parent only mounts this modal when
// canApproveUnlock is OK; the inline validation here is defense in
// depth.
//
// Props:
...
export default function UnlockApprovalModal({
  scenario,
  currentUser,
  hasApproveUnlock,
  onCancel,
  ...
  async function handleConfirm() {
    ...
    setSubmitError(null)
    try {
      const { error } = await supabase.rpc('approve_tuition_scenario_unlock', {
      ...
      >
        ...
        <div className="px-6 py-5 overflow-y-auto">
          ...
          <p className="font-body text-sm text-body leading-relaxed mt-3 pt-3 border-t-[0.5px] border-card-border">
            ...
            history; tuition agreements already signed against the
            locked rates remain in force.
          </p>

          {!validation.ok && validation.reason && (
            <p className="font-body text-status-amber text-sm italic mt-3" role="alert">
              {TUITION_LOCK_REASON_COPY[validation.reason] || 'You cannot approve this request.'}
            </p>
          )}
          {submitError && (
