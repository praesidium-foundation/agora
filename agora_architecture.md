...
**Version 3.8.16** — May 4, 2026 — Tuition Audit page redesign + operator-triggered snapshots + architectural correction (Stage 2 is not a lockable artifact). Full version history at the end of this document.
...
**Snapshot reason taxonomy** (CHECK constraint on `tuition_worksheet_snapshots.snapshot_reason`):

- `lock` — created by `lock_tuition_scenario` (Stage 1 only; Stage 2 never invokes the lock RPC)
- `midyear_reference` — operator-captured mid-year reference point
- `fall_semester_end` — operator-captured at end of fall semester
- `spring_semester_end` — operator-captured at end of spring semester
- `school_year_end` — operator-captured at end of school year
- `other` — operator-captured with a required free-text label

**RPC**: `capture_tuition_audit_snapshot(p_scenario_id, p_snapshot_reason, p_snapshot_label)` (Migration 038). SECURITY DEFINER; requires `tuition.edit`; validates Stage 2 / drafting state; copies every captured-by-value column on the scenario into a new snapshot row and every family_details row into snapshot_family_details (Migration 036's three new family columns included). Returns the new snapshot id. Does NOT change scenario state.

**Distinguishing lock-snapshots from operator-snapshots**: the `snapshot_reason` column. Lock snapshots also carry `locked_via = 'cascade' | 'override'`; operator snapshots carry `locked_via = 'snapshot'` (sentinel value used to satisfy the NOT NULL constraint inherited from the M023 schema; the Snapshots Panel UI uses `snapshot_reason` rather than `locked_via` to render the row's classification).
...
- **Migration 039**: Strategic Plan schemas (three instruments)
- **Migration 040**: Snapshot tables for Staffing, Enrollment (Budget shipped in 011; Tuition shipped in 023)
- **Migration 041**: Board Composition + Committees
- **Migration 042**: Org Acronyms registry, Custom KPI registry
- **Migration 043**: Module-to-Account mappings
- **Migration 044**: Workflow Configuration UI scaffolding (Phase R2) — depends on the Settings UI work; may add a dedicated `workflow_config` module-permission row
...
| Operator-triggered snapshots replace lock workflow for Stage 2 | Reference points within a Stage 2 scenario are captured via operator-triggered snapshots, not via lock events. The Capture Snapshot affordance in the page header opens a modal with a fixed reason taxonomy: `midyear_reference`, `fall_semester_end`, `spring_semester_end`, `school_year_end`, `other` (label required for the last). The `capture_tuition_audit_snapshot` RPC (Migration 038) creates an immutable snapshot of the current scenario state including all `family_details` rows; multiple snapshots may exist per scenario throughout the year. Lock-triggered snapshots (Stage 1 only) carry `snapshot_reason = 'lock'` for symmetry. Snapshots are read-only reference points (no second-identity approval required for capture — they document state, not governance decisions). | 7.3 |
