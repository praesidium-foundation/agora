...
**H1 / H2 split.** This session (H1) ships the DB layer: schema, three SECURITY DEFINER functions, the `approve_unlock` enum value, and the grant to the live user. The UI layer (request modal, approval banner with explicit "different approvers required" guidance, withdraw affordance, change_log surfacing of unlock events with distinct visual treatment) lands in H2 as a separate commit.
