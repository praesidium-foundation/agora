...
**Implementation choice**: Path B from design discussion. HTML-to-PDF rendering. The chosen mechanism for Phase 2 is `window.print()` + a print stylesheet (`@media print` rules in `src/index.css`); the browser opens its native print dialog and the user saves as PDF or prints directly. This avoids the infrastructure cost of a server-side headless-Chromium renderer (Puppeteer) on Vercel for a feature the user mostly uses ad-hoc. If emailable PDFs become a real requirement (e.g., scheduled reports, PDF attachments to board notification emails), Path B-server (Puppeteer in a Vercel serverless function rendering the same template) is the upgrade path; Layers 1 and 2 don't change. The print stylesheet is the template; future modules add their own `@media print` rules for their content.

**Commit E ships a stub print path**: View PDF on the Budget page calls `window.print()`; the print stylesheet hides the global chrome (top header, nav sidebar, KPI sidebar, action buttons, scenario tabs) so what prints is the budget detail itself. A `.draft-print-marker` element renders only on print and only for non-locked scenarios, providing the minimum DRAFT marking required by Section 2.5. **Commit F upgrades** the print layout: diagonal DRAFT watermark, header banner with school crest and AYE label, footer with locked-by/locked-at metadata for locked snapshots, hierarchical totals styling, narrative section, page breaks at category boundaries. Until then the print is functional but unpolished.
...
- **Migration 015**: Strategic Plan schemas (three instruments)
- **Migration 016**: Snapshot tables for Tuition, Staffing, Enrollment (Budget snapshots shipped in 011)
- **Migration 017**: Board Composition + Committees
- **Migration 018**: Org Acronyms registry, Custom KPI registry
- **Migration 019**: Module-to-Account mappings
- **Migration 020**: Workflow Configuration UI scaffolding (Phase R2) — depends on the Settings UI work; may add a dedicated `workflow_config` module-permission row
