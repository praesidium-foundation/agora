...
**Version 3.2** — April 29, 2026 — Phase 2 Commit G (documentation consolidation). Full version history at the end of this document.
...
**Two-step sequential approval.** Unlocking requires three distinct people: an initiator (who requests the unlock with justification) and **two distinct approvers** (who each independently confirm). The sequence is:

1. **Request.** Initiator with `submit_lock` permission calls `request_budget_stage_unlock(scenario_id, justification)`. Sets `unlock_requested = true` and stores the justification + initiator + timestamp on the scenario row.
2. **First approval.** A user with `approve_unlock` permission (who is NOT the initiator) calls `approve_budget_stage_unlock(scenario_id)`. Records approval 1; the function returns `'first_approval_recorded'`. The scenario remains in unlock-in-progress (still `state = 'locked'`).
3. **Second approval.** A different user with `approve_unlock` permission (who is NOT the initiator AND NOT the first approver) calls the same function. Records approval 2 (so the audit log captures who), then in the same transaction transitions `state` to `'drafting'` and clears all `unlock_*` fields. Returns `'unlock_completed'`.

Either path can be aborted with `reject_budget_stage_unlock(scenario_id, reason)`. Two authorization branches share this function: an `approve_unlock` holder can reject someone else's pending request; the original initiator can withdraw their own request without `approve_unlock`. Both paths require non-empty reason text. State stays `'locked'`; all unlock fields clear.

**State stays 'locked' throughout.** Migration 015's sibling-lock guards check `state = 'locked'`, and we want those guards to remain in force during the unlock-in-progress window. The active scenario IS still locked from the perspective of the AYE-stage slot; competing scenarios still can't claim recommended status or submit for lock review while it's locked. The flag fields (`unlock_requested`, `unlock_approval_1_*`, etc.) layer on top of `state = 'locked'`. The state machine value `pending_unlock_review` is reserved in the CHECK constraint but unused under this design — Decision: flags-on-top-of-locked is simpler than a full state machine state and avoids re-litigating the sibling-lock rules.

**Initiator separation enforced at three layers.** Defense in depth:

- **Schema:** Three CHECK constraints on `budget_stage_scenarios` — `unlock_initiator_not_approver_1`, `unlock_initiator_not_approver_2`, `unlock_approvers_distinct`. Plus `unlock_sequential_ordering` (approval 2 cannot populate before approval 1) and a BEFORE trigger `tg_unlock_only_when_locked` (the unlock_requested flag cannot be true unless state = 'locked').
- **Function:** `approve_budget_stage_unlock` raises a clear exception when the caller is the initiator or has already recorded approval 1.
- **(H2) Application + UI:** the validator + UI affordance layers will mirror the same rules so users see the block before they click. H1 lands DB only.

**Permission tier.** A new permission `approve_unlock` is added to the hierarchical `permission_level` enum, inserted between `approve_lock` and `admin`:

```
view < edit < submit_lock < approve_lock < approve_unlock < admin
```

Subsumption semantics — consistent with the rest of the permission system. `approve_lock` users do NOT auto-get `approve_unlock` (the `>=` check returns false). `admin` users do (admin subsumes everything). `approve_unlock` is granted manually per-user; no role inference. The grant is recorded once and survives indefinitely.

**Audit trail.** Every action in the workflow writes to `change_log` via the existing `tg_log_changes` trigger (attached to `budget_stage_scenarios` since Migration 011). The functions tag each transaction with a recognizable `app.change_reason` signature so audit-feed filters can group these events distinctly:

- `unlock_requested` — initial request
- `unlock_first_approval` — first-approval transition
- `unlock_completed` — second-approval transition (state flips here)
- `unlock_rejected: <reason>` — rejection by an approve_unlock holder
...
**Sibling-lock interaction during unlock-in-progress.** Migration 015's triggers (`tg_prevent_recommend_while_sibling_locked`, `tg_prevent_lock_submit_while_sibling_locked`) continue to fire while a sibling scenario in the same `(aye_id, stage_id)` is `state = 'locked'` — including throughout the unlock-in-progress window. Drafting alternatives remains allowed; promotion-class actions on siblings remain blocked until the second-approval transaction completes and state actually transitions to `'drafting'`.

**Cascade rules.** Unlock has no upstream cascade requirements and no downstream consumer enforcement today (§3.4). Architecture extension point only — not implemented in H1. When Final Budget or Strategic Plan ship and reference locked Budget snapshots, unlock-time validation will need to either block or warn when downstream consumers exist.

**H1 / H2 split.** H1 shipped the DB layer: schema, three SECURITY DEFINER functions, the `approve_unlock` enum value, and the grant to the live user. H2 (this session) shipped the application validator + UI layers — the second and third of the three-layer enforcement model. Both layers are codified below.
...
- `getUnlockBannerState(scenario)` — returns `'locked_no_request'`, `'locked_awaiting_first_approval'`, or `'locked_awaiting_final_approval'`. Single source of truth for which banner state to render.
- `canRequestUnlock(scenario, currentUser, hasSubmitLock)` — mirrors the DB rules in `request_budget_stage_unlock`.
- `canApproveUnlock(scenario, currentUser, hasApproveUnlock)` — mirrors `approve_budget_stage_unlock`, including initiator-separation and two-distinct-approvers checks.
...
- `UNLOCK_TEXT_MIN_LENGTH` (constant: 10 characters) — the client-side floor on justification / reason text. Guards against obvious throwaway text before round-tripping; the DB function only enforces non-empty after trim.

All `can*` helpers return `{ ok: true }` or `{ ok: false, reason: '<short_code>' }`. Reason codes are short, machine-readable identifiers (e.g. `'is_initiator'`, `'permission_insufficient'`) that the UI translates via `UNLOCK_REASON_COPY` — keeps logic and copy decoupled, so tests and audit messages can re-use the same conditions without coupling to user-facing strings.

**Permission shape note.** The prompt's helper signatures originally specified a `userPermissionLevel` string with hierarchy comparison logic. The actual codebase pattern is `useModulePermission(moduleCode, level)` returning `{ allowed: boolean }` per gate — the `>=` comparison is done server-side by `current_user_has_module_perm`. So budgetUnlock.js takes pre-evaluated booleans (`hasSubmitLock`, `hasApproveUnlock`) instead of attempting client-side hierarchy comparison. Semantically identical to the prompt's intent.

#### UI layer

**Single morphing banner.** `LockedBanner` extends to render three states determined by `getUnlockBannerState(scenario)`:

- `locked_no_request` — green/approved treatment (or amber if `locked_via='override'`). Existing production copy. "Request unlock" button appears when `canRequestUnlock(...).ok === true`.
- `locked_awaiting_first_approval` — amber treatment (`bg-status-amber-bg` / `text-status-amber`), 🔓 icon. Inline content: requester name + timestamp + full justification text (no truncation, parallel to override-event treatment, §9.1 commitment). Action buttons: "Approve unlock", "Reject", "Withdraw request" — each gated by the corresponding `canX` helper. The Approve button is rendered as disabled-with-tooltip (rather than hidden) when the user has `approve_unlock` but is blocked by initiator-separation or first-approver-self — so the user understands the rule rather than wondering where the affordance went.
- `locked_awaiting_final_approval` — same amber treatment, plus a second line showing the first approver and approval timestamp. Same action buttons; "Approve unlock (final)" button copy emphasizes that this approval transitions state.
...
- `RequestUnlockModal` — required justification textarea with live character counter (10-char minimum), submits to `request_budget_stage_unlock` RPC.
- `ApproveUnlockModal` — title and copy adapt to position. First-of-two: "Approve unlock — first of two", emphasizes that the scenario stays locked until a second different approver confirms. Final: "Approve unlock — final approval", emphasizes the state-transition consequence ("Approving now will return this scenario to drafting"). Submits to `approve_budget_stage_unlock` RPC; the modal closes on either return value (`'first_approval_recorded'` or `'unlock_completed'`) and the parent refetches.
...
- `WithdrawUnlockModal` — required reason textarea; muted/secondary button styling. Submits to the same `reject_budget_stage_unlock` RPC. **The same RPC handles both reject and withdraw paths** — H1's function auto-detects which path by comparing `auth.uid()` to `scenario.unlock_requested_by`. The two modals are deliberately distinct UI even though they share the RPC: rejection is a governance action (denying someone else's request) and gets destructive styling; withdrawal is housekeeping (cancelling your own request) and gets muted styling.

User display names (`unlock_requested_by`, `unlock_approval_1_by`) are resolved from `user_profiles.full_name` inside the components that need them (`LockedBanner`, `ApproveUnlockModal`, `RejectUnlockModal`) rather than plumbed through props from `BudgetStage` — keeps the lookup co-located with the only views that use them.

**Audit log integration.** `src/lib/auditLog.js` extended with five new event kinds (`unlock_requested`, `unlock_first_approval`, `unlock_completed`, `unlock_rejected`, `unlock_withdrawn`). `classifyEvent` detects these via the `change_log.reason` signature (set by H1's `app.change_reason` calls) BEFORE field-based heuristics — necessary because `unlock_completed` transitions state from `'locked'` → `'drafting'` which doesn't match any state-based pattern and would otherwise fall through to `'edit'`. `summarizeEvent` humanizes each kind: unlock_requested includes the full justification text inline; unlock_rejected and unlock_withdrawn extract the user's reason text from the `'unlock_rejected: <text>'` / `'unlock_withdrawn: <text>'` reason marker via the new exported `extractUnlockReasonText` helper. Visual treatment in `LineHistoryModal`, `ActivityFeedPanel`, `BudgetActivityPrint`, and `BudgetLineHistoryPrint`: amber for in-progress (parallel to override events), blue with 🔓 icon for `unlock_completed` (mirrors lock events as governance milestones at the symmetric ends of the lock/unlock cycle), red-muted for rejected, plain muted for withdrawn (housekeeping, neutral).
...
- **Migration 020**: Strategic Plan schemas (three instruments)
- **Migration 021**: Snapshot tables for Tuition, Staffing, Enrollment (Budget snapshots shipped in 011)
- **Migration 022**: Board Composition + Committees
- **Migration 023**: Org Acronyms registry, Custom KPI registry
- **Migration 024**: Module-to-Account mappings
- **Migration 025**: Workflow Configuration UI scaffolding (Phase R2) — depends on the Settings UI work; may add a dedicated `workflow_config` module-permission row
...
| Module workflows | Hybrid: school display name × Praesidium-curated stage type | 3.7 |
...
| Unlock approval count | Two distinct approvers, sequential | 8.13 |
| Unlock initiator separation | Initiator cannot be either approver | 8.13 |
| Unlock state model | State stays 'locked' during request; flag fields track in-progress | 8.13 |
| Unlock permission tier | Distinct from approve_lock; user-granted; subsumption-friendly placement between approve_lock and admin | 8.13 |
