...
**Version 3.8.21** — May 5, 2026 — Tuition-AuditPolish: viewport-responsive table height + editable Multi-Student Discount with override pattern. Full version history at the end of this document.
...
- Multi-Student Discount column displays $0 in the Stage 2 editor (the rule says it does not apply)
...
- **Faculty rows** still render `$0` for Multi-Student Discount and the cell is **not editable** (faculty rule precludes the discount). Toggling `is_faculty_family` true clears any prior `multi_student_discount_amount` override on the row; toggling false also clears the override (so auto-fallback drives the new non-faculty computation until the operator explicitly overrides again).
...
- **Cookson-class one-offs** (faculty family with pre-signed Tuition Agreement before faculty hire, where multi-student tier discount was honored on the signed math) compose two existing affordances: the operator either (a) keeps the family marked faculty and overrides Faculty Discount to the actual signed amount (which folds the multi-student-equivalent into the Faculty Discount value), or (b) marks the family non-faculty in the audit (the audit captures realized billing, not HR status) and uses Multi-Student auto-compute plus a separate Faculty Discount entry on a non-faculty row. Both paths produce the correct NET / YEAR; the choice depends on the operator's mental model of the family. No special-case import rules are needed.
