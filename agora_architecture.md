...
**Version 3.8.25** — May 5, 2026 — Tuition-AuditSavePersistence: per-family editor's Multi-Student override now round-trips correctly on refresh. Full version history at the end of this document.
...
#### AYE / school year / per-family enrollment dates (v3.8.14)

Three temporal concepts coexist in Agora and must not be confused:

1. **AYE / fiscal year** — 7/1 → 6/30. The container for planning, contracting, and accounting. Defined in the `academic_years` schema. All module scopes above reference the AYE as their time unit.
2. **School year / academic calendar** — varies annually; defined by the school's calendar. For Libertas this is approximately Sept 8 → May 29. There is no system-known representation today; the school's SMS / calendar holds it. A future Governance Calendar module (Phase 8) will configure school-year start / end dates per AYE; once configured, those dates can default downstream date pickers.
3. **Per-family enrollment dates** — `date_enrolled` and `date_withdrawn` on `tuition_worksheet_family_details` rows (v3.8.14, Migration 036). Operator-entered raw dates with null defaults; the system does not infer school-year boundaries today.

**Stage 2 audit semantics depend on actual school-year enrollment**, not fiscal-year boundaries. Families enrolled over summer who never attended are out of scope for the audit (handled in the school's SMS, not Agora). Pro-ration of mid-year enrollment is operator-handled via the discount fields and Notes context — not automated by the system. When the Governance Calendar module ships, the per-family date pickers can default from the configured school-year start date and the audit can surface "enrolled before school year start" / "withdrew before school year end" warnings.
...
tuition_worksheet_scenarios
  ...
  tier_rates                          jsonb               -- [{tier_size, per_student_rate, discount_pct, applies_when_n_students}, …]
                                                          --   v3.8.9 (B1.6): discount_pct field added; numeric
                                                          --   2-decimal precision (validated at the application
                                                          --   layer). Tier 1 (lowest tier_size) always has
                                                          --   discount_pct = 0 (the BASE rate); tiers 2+ store
                                                          --   the user-entered discount, and per_student_rate
                                                          --   is computed Math.round(base × (1 − discount/100))
                                                          --   on save via the cascade in TierRatesSection.
...
#### Tier Rates editability model

(v3.8.9 / B1.6) The Tier Rates section is configured to match the operational reality of tuition rate-setting. The user enters the base per-student rate (Tier 1) and the percentage discount for each higher tier; the per-student rates for tiers 2+ are computed from the base rate and the tier's discount percentage, displayed read-only and rounded to whole dollars.

**Editability:**

- **Tier 1 (BASE)** — `per_student_rate` is the editable input. The DISCOUNT column cell renders empty (the BASE badge in the Tier column conveys the meaning); `discount_pct` is stored as 0 always.
- **Tiers 2+** — `discount_pct` is the editable input (numeric, 2-decimal precision; "%" suffix on display). `per_student_rate` is read-only, computed as `Math.round(base_rate × (1 − discount_pct / 100))`.

**Cascade.** Editing the base rate cascades to all tier 2+ `per_student_rate` values atomically — the section recomputes every tier's rate from its stored `discount_pct` and writes the entire `tier_rates` jsonb in one save. Editing a single tier's `discount_pct` recomputes only that tier's `per_student_rate`. The cascade is user-visible: the user types a new base rate, tabs away, and sees all tier 2+ cells update simultaneously.

**New tiers** seed at `discount_pct = 0%` with `per_student_rate = base_rate`, requiring the user to enter a real discount intentionally. **Removing a tier** has no cascade implications — remaining tiers' `discount_pct` values are independent.

**Persisted shape.** The jsonb stores both the discount percentage (the input) and the computed per-student rate (the display value) — the rate is stored alongside the discount for snapshot fidelity and downstream read efficiency. Two-decimal precision on `discount_pct` matches the precision of school-side budgeting spreadsheets and round-trips dollar values cleanly: an existing scenario at $6,720 / $6,615 / $6,300 / $5,775 backfills to discounts of 0% / 1.56% / 6.25% / 14.06% and recomputes back to identical dollar values.

**No DB validators.** The application save path is the source of truth for `per_student_rate` (computes atomically from `discount_pct` on every save). DB-level validation of "tier 1 discount must be 0" or "discount must be < 100" was deliberately omitted — adds trigger complexity without much safety. The `tg_validate_tuition_scenario` from Migration 027 (breakdown_pct sum check) stays as-is.

**Workflow rationale.** The prior model (all per-student rates editable) required users to manually compute tier rates with a calculator while toggling between scenarios — exactly the friction the tool exists to eliminate. The new model matches the legacy Google Sheet's tier-rate input pattern (base + discount %) and lets the user iterate scenarios by changing one number at a time.
...
- `is_faculty_family` (per family, v3.8.14) — operational flag identifying which families receive the faculty discount. Toggling cascades `applied_tier_size`, `applied_tier_rate`, and `faculty_discount_amount` per the faculty discount rule (Appendix C v3.8.14).
- `date_enrolled`, `date_withdrawn` (per family, v3.8.14) — mid-year enrollment dates. Operator-entered raw dates with null defaults; the system does not infer school-year boundaries (see §3.3 for the AYE / school year / per-family enrollment dates distinction).
...
#### Faculty discount rule (v3.8.14)

The standard rule, codified in v3.8.14 (Tuition-B2a) after a real-data design discovery against Libertas's existing audit spreadsheet: **faculty discount REPLACES the multi-student tier discount**. Faculty families are charged base_rate × students × (1 − faculty_discount_pct/100); the multi-student tier discount does NOT apply to faculty families. Implementation:

- `applied_tier_size` resolves to 1 for faculty families regardless of headcount
- `applied_tier_rate` resolves to base_rate for faculty families
- Multi-Student Discount column **defaults** to $0 for faculty rows (the rule says it does not apply); **as of v3.8.22 the cell remains editable** — operators can override the default for Cookson-class one-off cases (faculty hires with pre-signed Tuition Agreements that honored the multi-student tier discount). The auto fallback is $0 for faculty; any non-null `multi_student_discount_amount` on a faculty row is treated as an operator override and surfaces with the gold-dot indicator.
- `faculty_discount_amount` auto-populates from base × students × pct on row creation, on `is_faculty_family` toggle, and on `students_enrolled` change (when not manually overridden)

Manual override is supported for non-standard cases (the spreadsheet's Cookson example, where faculty discount applied AFTER multi-student tier discount due to FA / contract signing timing). When the operator overrides `faculty_discount_amount` or `applied_tier_rate`, the override is visually marked with a small gold dot **inline-adjacent to the cell value** (format: `● ($X,XXX)` with the dot in brand gold; v3.8.16 repositioned from the prior corner-absolute placement) with a hover tooltip showing the auto-computed value. Override detection is render-time: compare stored vs. fresh-computed; divergence drives the indicator. Storage is Option B (computed value persists to the column on auto-cascade events; "override" means stored ≠ auto).

#### Multi-Student Discount override (v3.8.21)

The Multi-Student Discount column on the per-family editor was computed-only through v3.8.20. Real-data design discovery surfaced that operators occasionally need to record an actual realized multi-student discount that diverges from the auto-computed tier-discount math (one-off cases where a family's signed Tuition Agreement honored math different from current tier rules). v3.8.21 makes the column **editable for non-faculty rows** following the same Override-capable projection pattern (Appendix C, v3.8.20) that Faculty Discount used in B2a.

- **Schema** (Migration 045): `tuition_worksheet_family_details.multi_student_discount_amount numeric(12,2) NULL` (mirrored on `tuition_worksheet_snapshot_family_details`). NULL means auto-compute via `getFamilyMultiStudentAutoValue`; non-null is the operator override.
- **Math cascade decision** (Q-cascade-1, v3.8.21): when Multi-Student is overridden, **Net Tuition Rate stays at the canonical tier-derived value** — the override does NOT propagate to the Net Tuition Rate display. Subtotal Tuition for Year is computed as `base_rate × students_enrolled − effective_multi_student_discount` (so the override flows into Subtotal and NET / YEAR). This means the math display has a subtle inconsistency in the override case: `Net Tuition Rate × Enrolled` does NOT equal Subtotal. That's intentional — the divergence is the override's signal; the operator has stated "the actual subtotal isn't what tier math would produce." The Notes column captures the why per audit-trail-grade governance context.
- **Cascade contract on `is_faculty_family` toggle**: toggling either direction clears any prior `multi_student_discount_amount` override. Rationale: the auto-value context changes dramatically on toggle (faculty: 0; non-faculty: tier-derived), so a stale override anchored to the prior context no longer reflects operator intent. The operator can re-enter the override intentionally if it remains meaningful.

**v3.8.22 amendment — faculty-row override capability.** v3.8.21 originally restricted the editability to non-faculty rows, treating the faculty rule as a hard block on override. Real-data discovery during the v3.8.21 walkthrough surfaced that this was wrong: the Cookson-class one-off case (faculty family with pre-signed Tuition Agreement that honored the multi-student tier discount before faculty status changed) requires a direct override on the faculty row to align with the operator's mental model. v3.8.22 promotes the faculty rule from a hard block to a **default**: the auto value for faculty remains `$0` (the rule still preserves the standard semantics), but the cell is editable on faculty rows just as it is on non-faculty rows. Override detection works uniformly — any non-zero stored value on a faculty row diverges from the $0 auto value and surfaces the gold-dot indicator. This collapses the prior compositional workaround (mark family non-faculty in the audit, then add a separate Faculty Discount entry) into a single direct affordance: keep the family faculty, override Multi-Student Discount to the realized amount, and the math flows correctly through Subtotal and NET / YEAR. No schema change — the v3.8.21 column already supports the value; v3.8.22 is purely an application-layer relaxation in `tuitionMath.js`'s `computeFamilyMultiStudentDiscount` and `isFamilyMultiStudentOverridden` plus removal of the faculty/non-faculty branch in the table cell rendering. The cascade contract on `is_faculty_family` toggle is preserved (still clears the override on either direction) for the same context-shift reason.

#### Viewport-responsive table height (v3.8.21)

The family table's max-height is now responsive to the viewport AND to the reference-card-grid collapse state. The expanded-header reservation (~24rem) accommodates page header + breadcrumb + 3-card grid + action bar; the collapsed-header reservation (~13rem) reclaims the ~280-300px the card grid was using. A `min-h-[300px]` floor prevents the table from collapsing below ~5 rows on small viewports (internal scroll engages at that point instead). Sticky header behavior preserved. On a 1920×1200 screen, ~25-35 rows are visible expanded; on a 1366×768 laptop, ~8-12 rows after chrome.

#### Stage 2 working-document model (v3.8.16)

**Architectural correction: Tuition Audit (Stage 2) is NOT a lockable artifact.** Earlier build sequencing (B2b on the roadmap) had Stage 2 going through a Submit → Approve → Lock workflow parallel to Stage 1. Real-data design discovery during the B2a walkthrough surfaced that this is wrong: Stage 2 is a **living working document** maintained throughout the academic year. Families are added through summer pre-enrollment, mid-year transfers happen, families withdraw, financial aid committee decisions are recorded as they happen. There is no single moment at which "the audit is done and gets locked"; the audit IS the running record.

The corrected model (v3.8.16):

- The Stage 2 scenario stays in `state = 'drafting'` indefinitely. There is no submit-for-lock-review, no approve-and-lock, no two-identity unlock cycle. The Stage 2 lock UI placeholder shipped in B2a is removed.
- Reference points are captured via **operator-triggered snapshots**. A "Capture Snapshot" affordance in the page header opens a modal where the operator picks a reason and provides a label; the snapshot preserves the current state at the moment of capture.
- Multiple snapshots may exist per scenario throughout the year. The Snapshots side panel (in the page header) lists them chronologically; each snapshot is read-only and can be revisited later.

**Snapshot capture: freeform label model (v3.8.17 simplification).** Snapshots are operator-triggered via the Capture Snapshot button and require a single freeform `snapshot_label` text field — there is no fixed taxonomy of valid reasons. The school's operational tempo is captured via the label content rather than via system-enforced categories. Migration 038 introduced a CHECK-constrained `snapshot_reason` taxonomy ('midyear_reference' / 'fall_semester_end' / etc.); the B2-final walkthrough surfaced that schools have varying calendars and the taxonomy added rigidity without analytical value. Migration 039 dropped the CHECK constraint and simplified the RPC to a two-arg signature.

**RPC**: `capture_tuition_audit_snapshot(p_scenario_id, p_snapshot_label)` (Migration 039). SECURITY DEFINER; requires `tuition.edit`; validates Stage 2 / drafting state and non-empty label; copies every captured-by-value column on the scenario into a new snapshot row and every family_details row into snapshot_family_details (Migration 036's three new family columns included). Returns the new snapshot id. Does NOT change scenario state.

**Recent Activity surfacing.** Migration 039 also fixed the audit-log gap that prevented snapshot capture from appearing in the Recent Activity feed. The natural `tg_log_changes` row created on `tuition_worksheet_snapshots` INSERT was invisible to the activity feed because Tuition-D's `MODULE_AUDIT_CONFIGS.tuition` only queries `change_log` rows with `target_table = 'tuition_worksheet_scenarios'`. The fix: `capture_tuition_audit_snapshot` now writes a synthetic change_log row at the end of its body with `target_table = 'tuition_worksheet_scenarios'`, `field_name = '__snapshot_captured__'`, and `new_value = jsonb_build_object('snapshot_id', ..., 'label', ..., 'captured_at', ...)`. The application-side `classifyEvent` + `summarizeEvent` in `auditLog.js` recognize this synthetic event and render it as "Snapshot captured: \"\<label\>\"" in the activity feed. Class-of-issue note (per v3.8.12 audit-discipline lesson): RPCs that write to tracked tables must ensure the audit trail surfaces in the user-facing activity feed; the lock RPCs do this naturally via state UPDATEs on the scenario row, but a direct change_log INSERT is the right path when there is no natural state change.

**Distinguishing lock-snapshots from operator-snapshots**: lock snapshots carry `locked_via = 'cascade' | 'override'` and `snapshot_reason = NULL` (or `'lock'` if backfilled); operator snapshots carry `locked_via = 'snapshot'` (sentinel value used to satisfy the NOT NULL constraint inherited from the M023 schema) and `snapshot_reason = NULL` (taxonomy dropped in v3.8.17). The Snapshots Panel UI infers classification from `locked_via`; the freeform `snapshot_label` carries the operator's intent. The `snapshot_reason` column is preserved as a nullable vestigial field for back-compat with any test snapshots taken between Migrations 038 and 039 — a future cleanup migration may DROP COLUMN once confirmed unused.

**Why no Stage 2 lock**: the architectural reasoning that originally motivated Stage 2 lock — "Final Budget cascade should require Stage 2 locked" — was based on a misreading of operational reality. Final Budget reads from the locked Stage 1 snapshot for tuition revenue projections. The Stage 2 audit's running-actuals data is not a Final Budget input; it's the source of truth for variance reporting against Stage 1 projections, and variance reporting reads live from Stage 2 (or from any captured Stage 2 snapshot). No locked artifact is needed.

#### Bulk family import (v3.8.18)

The Stage 2 editor supports bulk import of family rows via XLSX or CSV upload, with a staging review step before commit. Operationally significant for schools doing initial start-of-year audits (Libertas's AYE 2026 has 84 families and 122 students; entering one-by-one is feasible but tedious). The import workflow is the operational on-ramp for Tuition Audit.

**Workflow**:

1. Operator clicks "+ Import CSV" in the Tuition Audit action bar → upload modal opens.
2. Modal step 1: Download Template — generates a multi-tab XLSX (Instructions tab with column descriptions, important rules, and live reference values pulled from the active scenario; Family Data tab with formatted column headers). Filename: `Libertas_Agora_{AYE label}_Tuition_Audit_Import_Template.xlsx`.
3. Operator fills out the Family Data tab in their spreadsheet app, saves as XLSX or CSV.
4. Modal step 2: Choose File — uploads the file. Client-side parser (`src/lib/tuitionImportParser.js`) normalizes display column names → snake_case schema columns, parses dates / currency / booleans with format-fallback, and produces a per-row package with `errors` and `warnings` arrays.
5. Client posts the parsed rows to `create_tuition_audit_import_batch(p_scenario_id, p_file_name, p_file_format, p_parsed_rows)` RPC. The RPC re-validates server-side (defense in depth) and inserts into `tuition_audit_import_batches` + `tuition_audit_import_staged_rows` (Migration 040 schema). Returns the new batch_id.
6. Browser navigates to `/modules/tuition/audit/import/{batch_id}` — the staging review page.
7. Staging UI (`TuitionAuditImportStaging.jsx`) shows: summary callout (total / errors / warnings / clean counts); mode radio buttons (append / replace); the staging table (`TuitionImportStagingTable.jsx`) with computed columns alongside parsed values; envelope-tracker preview showing post-accept envelope state given the mode toggle; Reject / Accept action buttons.
8. Operator chooses mode and clicks Accept → `accept_tuition_audit_import_batch(batch_id, mode)` RPC. Replace mode shows a confirmation modal first (destructive). The RPC validates no row has parse_errors, applies the faculty cascade rules (Stage 2 immutable fields immutable per Migration 022 trigger; tier_size and tier_rate auto-resolved from the scenario's tier_rates jsonb), inserts into `tuition_worksheet_family_details`, marks the batch accepted, and writes a synthetic change_log row pointed at the scenario for Recent Activity surfacing.
9. Browser navigates back to the Tuition Audit page; toast confirms the row count committed.

**Staging table architecture**: mirrors `TuitionFamilyDetailsTable`'s 13-column layout for visual continuity, plus a leading "Row #" column (1-indexed source spreadsheet row for operator reference) and a trailing "Status" column (Clean / N errors / N warnings; clicking expands inline to show detailed messages). Hard errors render with a red left rule; warnings with an amber rule; clean rows render plain. Editing-in-staging is out of scope for v3.8.18 — operators reject + re-upload to fix.

**Parse rules** (codified in Appendix C):

- **Dates**: accept MM/DD/YYYY, M/D/YYYY, MM/DD/YY, M/D/YY, YYYY-MM-DD. Try each format in priority order; first match wins; unparseable → parse error. SheetJS native Date instances pass through unchanged when `cellDates: true` reads the workbook.
- **Currency**: strict positive integer. Strip `$`, commas, whitespace. Parens or leading minus → soft warning ("Discount amounts should be entered as positive numbers; the system displays them as negative on the page"); the absolute value is staged so the operator can review.
- **Boolean** (Faculty column): liberal. TRUE/FALSE, true/false, 1/0, Y/N, Yes/No, T/F. Empty cell → false. Anything else → parse error.
- **Hard errors block accept** (server-side validation runs in `tg_validate_tuition_import_row` per Migration 041). Soft warnings advise but do not block. Validation runs server-side as the trust boundary; client-side validation is for UX (immediate feedback).

**Append vs Replace semantics**: chosen at acceptance time, not template time. Append preserves existing `family_details` rows and adds the staged ones. Replace deletes all existing `family_details` rows for the scenario in the same transaction as the INSERT (atomic; partial commits impossible). Replace mode shows a confirmation modal naming both row counts before submitting. Replace cannot be undone via the activity feed; the staged batch + the per-row change_log entries on the deleted rows preserve the historical trail.

**Audit-trail surfacing** (per v3.8.17 lesson): both `accept_tuition_audit_import_batch` and `reject_tuition_audit_import_batch` write a synthetic change_log row pointed at the parent scenario (`target_table = 'tuition_worksheet_scenarios'`, `field_name = '__import_accepted__'` or `'__import_rejected__'`, `new_value = jsonb_build_object('batch_id', ..., 'file_name', ..., 'mode', ..., 'row_count', ...)`). The `auditLog.js` helpers (`classifyEvent`, `summarizeEvent`) recognize the new event kinds and render them in the Recent Activity feed with descriptive summaries: "Imported 84 families (replace mode) from \"AYE2026_audit.xlsx\"" / "Rejected import \"<file>\". Reason: \"...\"".

**XLSX library choice**: SheetJS via the SheetJS CDN tarball (`xlsx-0.20.3.tgz`) — chosen over the npm-published `xlsx` (which has known CVEs with no fix available via npm) AND over alternatives like `exceljs` (which doesn't read CSV directly). The SheetJS CDN tarball is patched and zero-vulnerability per `npm audit`.

#### Final Budget reference snapshot (v3.8.20)

**The architectural keystone for cross-module KPI wiring.** Final Budget reads tuition data NOT from live Tuition Audit state but from a specific **operator-promoted Audit snapshot**. The mechanism:

- The operator captures Audit snapshots throughout the school year per the v3.8.16 working-document model.
- The operator promotes ONE snapshot per AYE as the "Final Budget reference" via a button in the Snapshots panel. The promotion is recorded in the activity feed (as a `snapshot_promoted` event with gold-rule treatment).
- Final Budget reads frozen data from the promoted snapshot — actual enrollment, NET tuition for year, family count — to drive its enrollment-dependent KPIs (Cost per Student, Tuition Gap, Break-even Enrollment, Number of Students).
- Live Audit edits do NOT shift Final Budget KPIs. Mid-year transfers, withdrawals, FA committee decisions continue to update the live Audit state but the Final Budget reference snapshot stays frozen until the operator explicitly re-promotes a different snapshot.

**Why the indirection.** Schools build Final Budget against a known enrollment baseline — typically the early-fall snapshot once enrollment finalizes. Final Budget should not move targets every time an audit edit happens; it should be deliberate, anchored, and explicitly re-anchored when the operator decides to. A direct read of live Audit data would conflate "what we plan to spend" (Final Budget construction) with "what we are seeing happen" (live Audit). The reference-snapshot indirection separates the two timelines and gives the operator full control over when Final Budget's view of enrollment changes.

**Schema** (Migration 043):

- `tuition_worksheet_snapshots.is_final_budget_reference bool NOT NULL DEFAULT false`
- Partial unique index `tuition_worksheet_snapshots_one_final_budget_reference ON (scenario_id) WHERE is_final_budget_reference = true` — enforces the one-promoted-per-scenario invariant.
- New RPC `mark_snapshot_as_final_budget_reference(p_snapshot_id)` — SECURITY DEFINER; validates Stage 2 + caller has tuition.edit; atomically demotes any prior reference for the scenario, then promotes the target snapshot. The demote-then-promote ordering avoids the unique-index conflict that would arise from a swap. Uses `set local session_replication_role = replica` to bypass `tg_prevent_snapshot_update` (the snapshot tables are immutable per Migration 023's trigger; same bypass pattern as Migrations 031 and 038's backfills). Writes a synthetic change_log row pointed at the scenario (`target_table='tuition_worksheet_scenarios'`, `field_name='__snapshot_promoted__'`, `reason='snapshot_promoted: <label>'`) for Recent Activity surfacing per v3.8.17 audit-trail-surfacing lesson.

**RPC layer** (Migration 044): two read-only functions for KPI consumers.

- `get_latest_locked_tuition_planning(p_aye_id)` — returns the locked Stage 1 snapshot's relevant fields (base_rate, total_students, total_families, breakeven_enrollment, faculty_discount_pct, curriculum_fee, ba_hourly_rate, captured_at, scenario_label, stage_display_name). Read by both Preliminary and Final Budget for "what we charge" data.
- `get_tuition_audit_final_budget_reference_summary(p_aye_id)` — returns the promoted Audit snapshot's aggregates: `total_students = SUM(students_enrolled)`, `total_families = COUNT(*)`, `net_tuition_for_year = SUM(applied_tier_rate × students_enrolled − faculty_discount − other_discount − financial_aid)` from `tuition_worksheet_snapshot_family_details`. The math mirrors `src/lib/tuitionMath.js`'s `computeFamilyNetTuition` per the v3.8.10 round-trip equivalence pattern: client and server compute the same number; the snapshot is the source of truth when both agree.

#### Cross-module data flow (v3.8.20)

Once Tuition is operationally complete, Preliminary Budget and Final Budget read from Tuition data sources via the RPC layer above. The flow is one-directional (Tuition feeds Budget; Budget never writes to Tuition tables) and snapshot-anchored (consumers read captured-by-value data, not live state).

**Locked Stage 1 Planning snapshot is the source for "what we charge" data**:

- `base_rate` (Tier 1 per-student rate) — used by both Preliminary and Final Budget for Current Tuition KPI
- `total_students` (Stage 1 enrollment projection) — used by Preliminary Budget for Number of Students KPI
- `breakeven_enrollment` (pre-computed at lock per Migration 032) — used by Preliminary Budget for Break-even Enrollment KPI
- `faculty_discount_pct`, `curriculum_fee`, `ba_hourly_rate` — informational; available to consumers if needed

**Promoted Tuition Audit snapshot is the source for "what we actually realized" data** (Final Budget only):

- `total_students` (actual headcount from per-family detail rows) — used by Final Budget for Number of Students KPI
- `net_tuition_for_year` (aggregate NET across all families) — used by Final Budget for the Break-even Enrollment KPI's avg-net-per-student denominator
- `total_families` (actual household count) — informational

**KPI computation locus**: client-side from cached reads. The KpiSidebar receives `tuitionStage1` and `tuitionAuditRef` as props (loaded once on AYE / stage change in BudgetStage); computes Cost per Student, Tuition Gap, and Break-even Enrollment from those refs plus the live `kpis.totalExpense`. As the operator edits Final Budget expense lines, the dependent KPIs recompute live (the upstream Tuition data stays cached and frozen until page reload). See Appendix C v3.8.20 decision row "Cross-module KPI consumers compute client-side from cached reads."

**Stage-aware sourcing**: the `KpiSidebar` consults `stageType` ('preliminary' | 'final') to decide which Tuition source drives each KPI:

| KPI | Preliminary Budget source | Final Budget source |
|---|---|---|
| Number of Students | Stage 1 `total_students` | Audit reference `total_students` |
| Current Tuition | Stage 1 `base_rate` | Stage 1 `base_rate` (same) |
| Cost per Student | `totalExpense / Stage 1 total_students` | `totalExpense / Audit ref total_students` |
| Tuition Gap | `Cost/Student − base_rate` | `Cost/Student − base_rate` |
| Break-even Enrollment | Stage 1 `breakeven_enrollment` (captured at lock) | `ceil(totalExpense / (Audit ref net_tuition_for_year / total_students))` (live recompute) |

Each KPI degrades gracefully to "—" + a "Pending …" subtitle naming the missing upstream when the source data isn't ready (no Stage 1 lock, no promoted reference, no expense entry).

**Future module integration**: when the Enrollment Estimator module ships, its locked enrollment estimate becomes the canonical source for `total_students` projection in Stage 1 of Tuition. The Stage 1 `total_students` field will gain operator-override capability — defaulting from Enrollment Estimator but editable when the Tuition Committee sees reason to project differently. See Appendix C v3.8.20 decision row "Override-capable projection pattern" for the generalized commitment.
...
The Tuition module's canonical authority pattern: the **Tuition Committee chair** (typically the Treasurer or HoS, depending on the school's committee structure) submits Stage 1 for review; the **Treasurer (or designee)** approves the Stage 1 lock. **Stage 2 has no lock workflow** under the v3.8.16 working-document model; per-family detail entry is performed by HoS or Office Manager throughout the year, and snapshots are captured by any user with `tuition.edit` (no second-identity approval required for snapshots — they are read-only reference points, not governance artifacts). The Financial Aid Committee operates outside system-level authority; its decisions appear as per-family annotations in Stage 2 family detail rows (the `notes` column captures "20% per Financial Aid Committee 6/2/25" as the audit signature).
...
#### Implementation status (Tuition-D, v3.8.10)

The Stage 1 lock and unlock workflows ship end-to-end, the Stage 2 setup gateway lands behind a cascade gate, and the cross-module Tuition→Budget cascade fires from the Budget side at submit time. The Stage 2 editing surface is deliberately deferred to a follow-on commit — gateway only for v3.8.10. Implementation cross-references:

- **Migration 032** — extends `tuition_worksheet_snapshots` with `kpi_breakeven_enrollment` (int), `kpi_net_education_program_ratio` (numeric(8,5)), `expense_comparator_amount_at_lock` (numeric(12,2)), `expense_comparator_source_label_at_lock` (text). Extends `compute_tuition_scenario_kpis` to compute the two new KPIs from `expense_comparator_amount`. Updates `lock_tuition_scenario` to capture the new KPIs and comparator metadata into the snapshot. The Stage 1 B&A revenue projection in `compute_tuition_scenario_kpis` now uses `projected_b_a_hours × hourly_rate` (Migration 024 hardcoded 0); the snapshot now records the projected B&A revenue alongside the actual at Stage 2.
- **`src/lib/tuitionWorksheet.js`** — application validator + gating helpers parallel to `budgetLock.js` and `budgetUnlock.js`. Exports `validateScenarioForLock`, `findLockedSibling`, `getLockBannerState`, the `canSubmitForLockReview` / `canApproveLock` / `canRejectLock` / `canRequestUnlock` / `canApproveUnlock` / `canRejectUnlock` / `canWithdrawUnlock` gates, the `validateUnlock*` content-validation wrappers, the three RPC wrappers (`submitTuitionScenarioForLockReview`, `approveAndLockTuitionScenario`, `rejectTuitionScenarioLock`), and `TUITION_LOCK_REASON_COPY` for end-user message strings.
- **Lock UI** (`src/components/tuition/`): `SubmitForLockReviewModal` (pre-flight modal with admin override path, mirrors Budget's `SubmitLockModal` but without cascade-rules call since Tuition has no upstream cascade). `LockReviewModal` (approver-facing; one modal hosts both Approve and Reject paths). `TuitionLockBanner` (unified component handling all three lock-state variants — pending, locked-no-request, locked-awaiting-final-approval — replacing Budget's two-component split for tuition's smaller page).
- **Unlock UI** (`src/components/tuition/`): `RequestUnlockModal`, `UnlockApprovalModal`, `RejectUnlockModal`, `WithdrawUnlockModal`. Each calls its respective RPC (Migration 025); UI shapes mirror Budget's modals with tuition-specific copy ("locked tier rates", "tuition agreements remain in force on unlock", etc.).
- **Stage 2 setup gateway** (`src/pages/modules/TuitionAuditPage.jsx` + `src/components/tuition/TuitionAuditEmptyState.jsx`): mounted at `/modules/tuition/audit`. Resolves the `final`-typed tuition stage, probes for any locked Stage 1 snapshot in the active AYE, and renders the empty-state with one of two messages (cascade blocked vs. ready-but-editor-pending). The Stage 2 editing surface itself is queued — the gateway is shipped first so the cascade is testable.
- **Cross-module cascade** (`src/lib/budgetLock.js`): `getTuitionStage1LockState(ayeId)` helper + `checkCascadeRules` extension. When called with `scenarioStageType: 'preliminary'`, `checkCascadeRules` injects a hardBlock failure when no Tuition Stage 1 snapshot exists for the AYE. Enforced at the application layer only (no DB trigger added in v3.8.10) — see Appendix C decision row "Cross-module cascade enforcement is prospective, not retroactive." `BudgetStage` passes `scenarioStage` through to `SubmitLockModal`, which forwards `scenarioStageType` to the cascade check.
- **Activity feed generalization** (`src/lib/auditLog.js`): `MODULE_AUDIT_CONFIGS` map keyed by module id; `fetchScenarioActivity` signature changed to `(moduleId, scenarioId, opts)`. Tuition entry maps `scenarioTable: 'tuition_worksheet_scenarios'` and `lineTable: null` (Stage 1 has no per-row line table — configuration lives inline in jsonb columns). `ActivityFeedModal` gains a `moduleId` prop; the Budget call sites and the Budget Activity print page were updated to pass `'budget'` as the new first arg. Tuition's worksheet page now opens the same modal with `moduleId="tuition"`.
- **Header + integration** (`src/pages/modules/TuitionWorksheet.jsx`): adds the "Submit for Lock Review" primary button, the lock-state banners, the recommended-scenario inline hint, the sibling-locked drafting banner, the wired Recent Activity link, and all six new modals (SubmitForLockReview, LockReview, RequestUnlock, UnlockApproval, RejectUnlock, WithdrawUnlock).
- **Sidebar** (`src/components/AppShell.jsx`): the Tuition Audit child is uncommented; clicking it routes to `/modules/tuition/audit`.

**What's deferred** (to be picked up in a future Tuition-E session):

- Tuition Schedule PDF (the family-facing artifact generated from the Stage 1 locked snapshot per §7.3 "Tuition Schedule PDF").
- Stage 2 editing surface — per-family rows, applied tier rate dropdown, Faculty/Other/FA per-family allocation columns, the audit comparison view against the Stage 1 snapshot. (**Editor itself shipped in B2a, v3.8.14**; comparison view against the Stage 1 snapshot remains queued.)
- Tuition Activity print route (`/print/tuition/:scenarioId/activity`) — the activity-feed modal renders without the PDF export link for tuition until the print route ships.
- Year-over-year comparison (Tuition-B3 work, separate session).
- Server-side hoist of additional KPI math beyond what Migration 032 captured.

#### Implementation status (Tuition-B2a, v3.8.14)

The Stage 2 per-family editor ships as the operational workspace. The data shape (Migration 022 base + Migration 036 additions) supports the legacy Google Sheet's column structure: 11 numeric / text columns + faculty toggle + 2 date columns. Faculty discount rule codified per Appendix C. Stage 2 lock workflow, envelope running totals header zone, and mid-year snapshot capability are queued for B2b. Implementation cross-references:

- **Migration 036** — extends `tuition_worksheet_family_details` (and the snapshot mirror) with three columns: `is_faculty_family bool NOT NULL DEFAULT false`, `date_enrolled date NULL`, `date_withdrawn date NULL`. No backfill needed; both tables had zero production rows. **Known gap** (documented in the migration header): `lock_tuition_scenario` (Migration 034) does not yet reference the three new columns in its snapshot INSERT. B2b's lock-workflow rewrite of `lock_tuition_scenario` must extend the snapshot capture.
- **`src/lib/tuitionMath.js`** — six new family-row helpers: `computeFamilyAppliedTierRate`, `computeFamilyMultiStudentDiscount`, `computeFamilyFacultyDiscountAuto`, `computeFamilyNetTuition`, `isFamilyFacultyDiscountOverridden`, `isFamilyTierRateOverridden`, plus exported `naturalAppliedTierSize` and `naturalAppliedTierRate` for the save-path cascade. The override-detection helpers compare the stored value against the auto-computed value (Option B storage pattern from Migration 027); divergence drives the gold-dot indicator.
- **`src/lib/auditLog.js`** — new `fetchFamilyNotesHistory(familyId)` helper. Filters `change_log` to `target_table = 'tuition_worksheet_family_details' AND target_id = familyId AND field_name in ('notes', '__insert__', '__delete__')`. Targeted lookup parallels Budget's `fetchLineHistory` but scoped to the notes column (the audit-trail-grade field on family rows).
- **`TuitionFamilyDetailsTable.jsx`** — the editor itself. Eleven primary columns (Family Name, # Enrolled, Base Tuition, Multi-Student Discount, Net Tuition Rate, Subtotal Tuition for Year, Faculty Discount, Other Discounts, Financial Aid, NET Tuition for YEAR, Notes), faculty toggle in a narrow leading column, two date columns, plus a row-actions column with the per-row clock affordance and a delete affordance. Sort: faculty families clustered alphabetically at the top with a thin navy-tinted separator rule between groups. Direct-edit-with-undo on save (B1 pattern); save on blur with optimistic local state and toast on error.
- **`TuitionFamilyHistoryModal.jsx`** — per-row notes history. Mirrors Budget's `LineHistoryModal` shape; clock icon (small navy SVG, no emoji) opens the modal scoped to the family's row. Notes-field events render with a "From: / To:" diff. Insert events show "Created with notes: …" framing. Locked-state suppression: clock icon hides when scenario state is locked (architecture §9.1).
- **`TuitionAuditPage.jsx`** — three-state branching: cascade-blocked (no Stage 1 lock), Stage 1 locked but no Stage 2 scenario (renders a setup card with "Set up Audit from {Stage 1 label}" button calling `create_tuition_scenario_from_snapshot` RPC), Stage 2 scenario exists (renders the editor). Stage 2 lock workflow placeholder note rendered in the header where the Submit-for-Lock-Review button will eventually live.
- **Visual override indicator**: small gold Unicode bullet (•) at the top-right of the Faculty Discount and Net Tuition Rate cells when the stored value diverges from the auto-computed value. Hover tooltip shows the auto value. Implemented as an inline `<span>` (not CSS pseudo-element) for tooltip accessibility. NO emoji icons anywhere on the page; the gold dot and the SVG clock icon are the only visual chrome additions. **(v3.8.16 update: gold dot repositioned from corner-absolute to inline-adjacent; format `● ($X,XXX)` with the dot rendered immediately before the cell value.)**

#### Implementation status (Tuition-B2-final, v3.8.16)

The Tuition Audit page is redesigned around the legacy Google Sheet's working-tool pattern: dense reference header (3-card grid: Tier Rates / Discount Envelopes / Enrolled Families) above a spreadsheet-density family table with sticky header and clear column groupings. Stage 2 lock workflow is **removed from scope** per the architectural correction in v3.8.16; operator-triggered snapshots replace it. Implementation cross-references:

- **Migration 038** — `tuition_worksheet_snapshots` schema additions: `snapshot_reason text` (CHECK on the taxonomy `'lock' | 'midyear_reference' | 'fall_semester_end' | 'spring_semester_end' | 'school_year_end' | 'other'`), `snapshot_label text` (operator-provided), `captured_at timestamptz NOT NULL DEFAULT now()`. New RPC `capture_tuition_audit_snapshot(p_scenario_id, p_snapshot_reason, p_snapshot_label)` is the operator-triggered snapshot path; SECURITY DEFINER, requires `tuition.edit`, copies every captured-by-value column on the scenario plus every `family_details` row (including Migration 036's three columns — closing the M036 known gap for the operator-snapshot path). `lock_tuition_scenario` is intentionally NOT modified — Stage 1 has no `family_details` rows so the M036 gap doesn't affect it. Legacy lock snapshots get backfilled `snapshot_reason = 'lock'` (no-op against empty production tables).
- **`src/lib/tuitionMath.js` extensions** — three header-zone aggregate helpers: `computeEnvelopesUsed(families, scenario)` returns `{rows, total}` driving the Discount Envelopes card; `computeFamilyDistribution(families)` returns `{tiers, totalFamilies, totalStudents}` driving the Enrolled Families card; `computeNetTuitionForYear(families, scenario)` returns the bottom-line aggregate. Plus four sum helpers (one per envelope category).
- **`src/lib/budgetLock.js`** — comment update only. The "Final Budget cascade requires Tuition Stage 2 locked" rule was an architectural assumption never enforced in code; under v3.8.16's non-lockable Stage 2 model it is formally retired. Final Budget continues to gate on Tuition module-level `'locked'` state via the existing `school_lock_cascade_rules` row, which fires when Stage 1 locks (Stage 1 lock transitions the tuition module instance to `'locked'`; Stage 2 is irrelevant to module instance state under the new model).
- **Lock-workflow placeholder removed** — the B2a `TuitionAuditPage` carried a "Stage 2 lock workflow ships in Tuition-B2b" italic note in the header. Removed in v3.8.16; replaced with the Capture Snapshot button + Snapshots / Recent Activity links.
- **`CaptureSnapshotModal.jsx`** — new modal opened by the Capture Snapshot button. Reason field (radio buttons matching the taxonomy); label field (required when reason is "Other", optional otherwise; auto-populated suggestion based on reason + AYE label, editable). Confirm calls the RPC; success toast names the captured snapshot.
- **`SnapshotsPanel.jsx`** — new right-side slide-out panel listing all snapshots for the active scenario chronologically (newest first). Each row shows reason, label, captured_at, captured_by_name. Per-row "View" button opens a placeholder toast directing users to the Tuition-E read-only snapshot view (deferred). Distinguishes lock snapshots from operator snapshots via `snapshot_reason` (or `locked_via` fallback for pre-M038 legacy data).
- **`TuitionFamilyDetailsTable.jsx` redesign** — 15-column layout matching the v3 mockup: `# | Family Name | Faculty | # Enr. | Base Tuition | Multiple Student Disc. | Net Tuition Rate | Subtotal for Year | Faculty Discount | Other Discount | Financial Aid Amount | NET Tuition for YEAR | Date Enrolled | Notes | Date Withdrawn`. Vertical column dividers (using `--col-divider: #D4CDB8`) at four group boundaries: Identity, Computed, Discount Allocations, Result. Sticky header (`position: sticky; top: 0` on `<thead>`). Header text in solid #192A4F navy at 11.5px Cinzel small-caps with two-line headers via `<br/>`. Row striping (alternating cream-tinted rows). `# Enr.` input has spinner arrows suppressed via `[appearance:textfield]` + `[&::-webkit-inner-spin-button]:appearance-none`. Date Withdrawn null displays as em-dash with a click-to-arm-picker affordance (no `mm/dd/yyyy` placeholder). Sort applies on dropdown change and first load only — mid-session edits and additions preserve the existing display order; new rows append at the end.
- **`TuitionAuditPage.jsx` redesign** — new layout: breadcrumb + title row with collapse chevron + page-action links (Capture Snapshot button, Snapshots, Recent Activity); 3-card reference grid (1:1.6:1 weighted) collapsible via the chevron with state persisted to localStorage per scenario; action bar (Sort By dropdown, + Add Family, + Import CSV); family table. The "+ Import CSV" affordance renders with dashed muted styling and shows a "coming in next session" toast on click — placeholder hook for Tuition-B2-import. Three-state page branching from B2a preserved (cascade-blocked / Stage 1 locked but no Stage 2 / Stage 2 scenario exists).
- **Sort options** (v3.8.16, controlled at the page level): Alphabetical (default), Date enrolled (newest first), Date enrolled (oldest first), Date withdrawn (most recent first; null at bottom), Faculty first. Sort applies on dropdown change and on initial load; mid-session edits and adds DO NOT trigger re-sort. New rows append at the bottom of the current display order.
- **Horizontal scroll fix** — table wrapper uses `overflow-x: auto` on the column-overflow case only. At 1920×1080 with 15 columns at the specified widths, no horizontal scroll appears. At 1366×768 (typical staff laptop), horizontal scroll engages cleanly.

**What's deferred** (queued for B2-import next session):

- CSV template generation, upload + parse + staging UI, bulk review/accept flow mirroring the COA pattern.
- Validation rules for imported family rows; faculty-rule warnings on import.

**What's deferred indefinitely**:

- Read-only snapshot view (full Tuition Audit page rendered against snapshot data) — Tuition-E.
- Year-over-year comparison view — Tuition-B3 (with Stage 2 not locking, this likely compares two operator-captured snapshots from different AYEs rather than comparing locked artifacts).
- Tuition Schedule PDF (family-facing) and Tuition Audit summary PDF — Tuition-E.
...
| Tuition Stage 2 (Audit) | Tuition Stage 1 locked (same AYE) | Stage-initialization cascade per §3.4: Stage 2 cannot be set up until Stage 1 is locked, since Stage 2 seeds from the Stage 1 snapshot. **Stage 2 itself never locks** under the v3.8.16 working-document model; see §7.3 "Stage 2 working-document model" for the operator-snapshot pattern that replaces the lock workflow. |
| Final Budget | Preliminary Budget locked (same AYE); Tuition Stage 1 locked; Staffing locked | **v3.8.16 update**: "Tuition Stage 2 locked" is removed as an upstream prerequisite (Stage 2 is not lockable). Final Budget reads tuition revenue projections from the locked Stage 1 snapshot; Stage 2's running-actuals data is the source for variance reporting against Stage 1 projections, not a Final Budget input. |
...
**Implementation status (v3.8.10).** The Tuition Stage 1 → Preliminary Budget cascade is enforced from the Budget side as of Tuition-D. `src/lib/budgetLock.js` exports `getTuitionStage1LockState(ayeId)` and the `checkCascadeRules` function takes an optional `scenarioStageType` parameter; when `scenarioStageType === 'preliminary'`, the cascade injects a hardBlock failure if no Tuition Stage 1 snapshot exists for the AYE. Application-layer enforcement only — no DB trigger added in v3.8.10 (the existing Migration 022 sibling-locked guard is the only DB-level cross-module check on the Tuition side; the Budget side's `lock_budget_stage_scenario` RPC does not consult Tuition state). The application-layer check fires at submit-for-lock-review time inside `SubmitLockModal`, alongside the existing `school_lock_cascade_rules` table-driven checks. The two enforcement surfaces are conceptually distinct: the table-driven rules (Strategic Plan adopted, etc.) are configurable per school; the per-stage cross-module rule is hardcoded because it expresses an architectural invariant (Stage 1 carries the tuition schedule the Budget should reflect) rather than a school-level policy choice. When the catalog of cross-module per-stage rules grows beyond this single rule, a `school_lock_cascade_rules_per_stage` table is the natural extension.

**Prospective vs. retroactive.** Cross-module cascade enforcement is **prospective**: the rule fires on lock submissions made AFTER v3.8.10 ships. AYE 2026's Preliminary Budget was locked-with-override before Tuition was wired up at all (and predates this rule by design — see §7.5's "Override paths exist" paragraph); the historical artifact is preserved as-is. The cascade fires the next time anyone goes through the lock flow on a Preliminary Budget — for AYE 2026 that means after an unlock + re-lock cycle, or when AYE 2027 reaches Preliminary Budget. This is the same prospective-enforcement principle that applied to Migration 015's sibling-lock guard at the time of its introduction.
...
### 9.8 Synthetic test user fixture

The Agora codebase relies on a synthetic test user, `treasurer.test@libertas.local`, as a recurring fixture for testing two-identity workflows. Created during Phase 2 Budget testing, the user holds `approve_unlock` permissions across Planning modules so that lock and unlock workflows requiring two distinct identities can be exercised end-to-end without coordinating two human testers. The user's purpose is **workflow validation, not data entry** — the natural testing pattern is "primary user (Jenna) edits and submits, synthetic user approves" or its inverse.

**Permission set as of v3.8.13:**

- **Budget**: `approve_unlock` (subsumes view, edit, submit_lock, approve_lock per the hierarchical enum)
- **Tuition**: `approve_unlock` (same subsumption)

**On the "no edit" intent vs. the hierarchical schema.** The ideal grant for a workflow-test fixture would be "view + submit_lock + approve_unlock without edit" — view to navigate, submit_lock for the submit-direction flow, approve_unlock to act as second approver, and not edit because the synthetic user is for workflow testing not data entry. The schema does not support that combination directly: the unique `(user_id, module_id)` constraint enforces one row per user per module, and the hierarchical `permission_level` enum (`view < edit < submit_lock < approve_lock < approve_unlock < admin`) means the granted level is a floor — every lower level is implicitly held via subsumption. The pragmatic resolution: grant `approve_unlock` (which subsumes edit) and rely on the testing convention "do not use the synthetic user for data entry" rather than schema enforcement. If a future need surfaces for true per-permission granularity (without subsumption), that's a schema-level redesign tracked in Appendix D.

**Maintenance discipline.** Every module that ships with a two-identity workflow (lock/unlock cycle, multi-approver gates, or similar) must grant the synthetic user `approve_unlock` on that module as part of the module's lock-workflow session acceptance criteria. The grant SQL is included in the same migration that ships the lock-workflow RPCs OR in a dedicated test-fixture migration ridden alongside (the latter is the v3.8.13 pattern via Migration 035). Future modules to extend grants for: Staffing, Enrollment Estimator, Tuition Audit (Stage 2 lock workflow when B2 ships), Final Budget cascade work.

**Credentials and account hygiene.** User credentials are documented in the project's secure operational notes (not in source control). Password rotation and account hygiene follow the school's standard practices. Re-creating the user in a fresh environment uses the standard Supabase auth provisioning flow; the test-fixture grant migrations are no-ops if the account doesn't yet exist (the SELECT against `auth.users` returns no rows; the INSERT inserts nothing).

**Why a single synthetic user, not many.** Agora's two-identity workflows (per §8.13) require precisely two distinct identities — the requester and the second approver. One synthetic user complementing the primary live user (Jenna) covers every two-identity test today. If a future need surfaces for multi-user scenarios beyond two-identity (multi-quorum approvals, multi-role gates, etc.), additional synthetic users get added as separate fixtures with their own architectural documentation; the v3.8.13 fixture is deliberately not generalized ahead of need.
...
**Cross-cutting checklist for any phase that ships a two-identity workflow** (lock/unlock cycle, multi-approver gates): grant the synthetic test user `approve_unlock` on the new module, either inline in the lock-workflow migration or in a dedicated test-fixture migration ridden alongside. See §9.8 for the synthetic-user fixture documentation. Migration 035 is the canonical example for Tuition.
...
- **Migration 031**: Tuition-B1.6 — Tier Rates editability flip; `tier_rates` jsonb gains `discount_pct` field (v3.8.9). Each row in the `tier_rates` jsonb on `tuition_worksheet_scenarios` (and mirrored on `tuition_worksheet_snapshots`) gains a `discount_pct` field (numeric, 2-decimal precision via application-side rounding). Backfill via PL/pgSQL DO block: tier 1 (lowest tier_size) sets `discount_pct = 0` unconditionally; tiers 2+ compute `discount_pct = ROUND(((tier_1_rate - this_tier_rate) / tier_1_rate) × 100, 2)` when `tier_1_rate > 0`, else 0. Per-student rate values preserved verbatim during backfill (no rounding, no recomputation — only adds the new field). Round-trip math (per_student_rate ↔ discount_pct ↔ per_student_rate) preserves the input dollar values exactly: AYE 2026 walkthrough scenario at $6,720 / $6,615 / $6,300 / $5,775 → discounts of 0% / 1.56% / 6.25% / 14.06% → recomputed back to identical dollar values. Snapshot mirror uses `set local session_replication_role = replica` to bypass `tg_prevent_snapshot_update` for the backfill (likely zero locked snapshots at present, but the migration handles them safely). No new DB validators added — the application save path is the source of truth for `per_student_rate` (computes atomically from `discount_pct` via the cascade in `TierRatesSection.jsx`); the existing `tg_validate_tuition_scenario` from Migration 027 stays as-is. No SECURITY DEFINER function changes — `lock_tuition_scenario` and `create_tuition_scenario_from_snapshot` write/read the `tier_rates` jsonb opaquely; the new field carries through without RPC body changes. Notify pgrst at the end.
...
- **Migration 032**: Tuition-D — snapshot KPI extension + comparator capture (v3.8.10). Adds four nullable columns to `tuition_worksheet_snapshots`: `kpi_breakeven_enrollment int`, `kpi_net_education_program_ratio numeric(8,5)` (5-decimal precision so 102.347% round-trips cleanly), `expense_comparator_amount_at_lock numeric(12,2)`, `expense_comparator_source_label_at_lock text`. Extends `compute_tuition_scenario_kpis` to compute the two new KPIs from the scenario's `expense_comparator_amount` (NULL comparator → both KPIs NULL — Stage 1 may legitimately be locked without a comparator selected). Math mirrors `src/lib/tuitionMath.js`'s `computeNetEdProgramRatio` and `computeBreakevenEnrollment`. Stage 1 B&A revenue projection now uses `projected_b_a_hours × hourly_rate` (Migration 024 hardcoded 0 for Stage 1 — that simplification is retired). Updates `lock_tuition_scenario` to capture all four new fields into the snapshot. The hybrid KPI computation discipline is preserved: drafting renders client-side via `tuitionMath.js`; lock-time captures server-side for snapshot fidelity. See Appendix C decision row "Server-side KPI computation at lock time, client-side at drafting time (hybrid)" for the rationale. **Note:** Migration 032's rewrite of `compute_tuition_scenario_kpis` inadvertently reverted three Migration 027 Stage 1 changes (column renames + `total_students` column read + `projected_faculty_discount_amount` direct read); Migration 033 is the hotfix.
- **Migration 033**: Tuition `compute_tuition_scenario_kpis` hotfix (v3.8.11). Migration 032 rewrote `compute_tuition_scenario_kpis` to add the Tuition-D decision KPIs (breakeven_enrollment, net_education_program_ratio) and the corrected B&A projection, but the rewritten function body referenced two pre-Migration-027 column names (`other_discount_envelope`, `financial_aid_envelope`) that no longer exist on `tuition_worksheet_scenarios`. The function compiled but failed at runtime when `lock_tuition_scenario` invoked it ("record 'v_scenario' has no field 'other_discount_envelope'"). Single CREATE OR REPLACE FUNCTION corrects the references to `projected_other_discount` and `projected_financial_aid`. Same pass also restored two other Migration 027 Stage 1 design decisions that the Migration 032 rewrite reverted: `total_students` is now read from the column directly (not summed from the jsonb walk), and `faculty_discount` reads `projected_faculty_discount_amount` directly (not computed as `pct × gross`). Both restorations are required for the snapshot KPIs to match the client-side sidebar within rounding tolerance — the snapshot-matches-sidebar acceptance criterion that v3.8.10 established. Preserved from Migration 032: the Stage 1 B&A revenue projection using `projected_b_a_hours × hourly_rate` (intentional v3.8.10 improvement), the OUT parameter list (11 columns; no DROP needed since shape is unchanged from the immediately-prior version), and the new decision KPIs. No schema change. notify pgrst at end.
- **Migration 035**: Tuition test fixture grants for synthetic user (v3.8.13). Grants `treasurer.test@libertas.local` `approve_unlock` on the Tuition module so the synthetic user can act as the second approver in two-identity Tuition unlock-cycle testing. Defensively re-grants `approve_unlock` on the Budget module — the original Phase 2 grant was set via SQL Editor and never captured in a migration; this restores it under version control. Both grants are idempotent (`ON CONFLICT DO UPDATE WHERE existing_level < approve_unlock`) — a no-op if the user already holds `approve_unlock` or higher on either module, and never demotes admin. The hierarchical `permission_level` enum means `approve_unlock` subsumes view/edit/submit_lock/approve_lock; the synthetic user therefore has implicit edit on both modules even though the testing convention is "synthetic user does not enter scenario data." See §9.8 for the fixture's full documentation. notify pgrst at end.
- **Migration 036**: Tuition-B2a — per-family editor schema (v3.8.14). Three new nullable / default-false columns on `tuition_worksheet_family_details` (mirrored on `tuition_worksheet_snapshot_family_details` for snapshot fidelity per §5.1): `is_faculty_family bool NOT NULL DEFAULT false` (drives the faculty-discount-replaces-multi-student-tier-discount rule per Appendix C v3.8.14), `date_enrolled date NULL` and `date_withdrawn date NULL` (mid-year enrollment tracking; operator-entered raw dates with no system-side school-year inference per §3.3 v3.8.14). No backfill — both tables had zero production rows. No new triggers. No new RPCs. **Known gap for B2b**: `lock_tuition_scenario` (Migration 034) currently copies family_details rows to snapshot_family_details with an explicit column list that does not yet include the three new columns. Stage 2 lock today (only triggerable via direct RPC call — there is no UI affordance until B2b) would lose the three fields on snapshot capture. B2b's lock-workflow rewrite must extend the snapshot INSERT; the audit-discipline lesson from Migration 034 applies (read the CURRENT function body, not an older migration). notify pgrst at end.
- **Migration 037**: Tuition `create_tuition_scenario_from_snapshot` parameter naming alignment (v3.8.15). Migration 026 created the RPC with parameter `p_scenario_label` (the third positional arg); the Tuition-B2a setup card in `TuitionAuditPage.jsx` called the RPC with `p_scenario_name` (the natural English match for "scenario name"). PostgREST resolves named-parameter RPC calls by exact name match — there is no name-fuzzing fallback — so the lookup raised "Could not find the function public.create_tuition_scenario_from_snapshot(p_scenario_name, p_source_snapshot_id, p_target_stage_id) in the schema cache" the first time the button was clicked in production. The error did not surface in dev because no test exercised the button between B2a's commit and the live attempt. Migration 037 drops the original function (parameter NAMES are part of the signature for PostgREST's argument-list match; CREATE OR REPLACE alone cannot rename them) and creates a new version with `p_scenario_name` matching the B2a call site. Function body preserved verbatim from Migration 029 — same validation, same field-copy list (M027 + M028 + M029 column additions all included), same SECURITY DEFINER, same search_path, same return type. Only the parameter name reference inside the body (`p_scenario_label` → `p_scenario_name`) changes. Audit-discipline lesson reinforced (per v3.8.12): when calling existing RPCs, verify parameter names match between the function definition and the call site. notify pgrst at end.
- **Migration 040**: Tuition-B2-import — staging tables (v3.8.18). Two new tables enabling the bulk-family-import workflow with staging review. **`tuition_audit_import_batches`** is the per-upload header carrying `scenario_id` (FK to tuition_worksheet_scenarios with ON DELETE CASCADE), `uploaded_by`, `uploaded_at`, `file_name`, `file_format` ('csv' | 'xlsx'), `row_count`, `status` ('staged' | 'accepted' | 'rejected' | 'expired'), `mode` ('append' | 'replace'; nullable while staged, required at accept), `accepted_at`, `rejected_at`, `rejected_reason`. A CHECK constraint enforces lifecycle consistency (e.g., a status='accepted' row must have accepted_at and mode populated). **`tuition_audit_import_staged_rows`** mirrors `tuition_worksheet_family_details` schema with all columns nullable in staging (validation surfaces missing fields via parse_errors rather than schema rejection so operators see broken rows in the staging UI). Adds `row_number int NOT NULL` (1-indexed source spreadsheet position for operator-facing display), `parse_errors jsonb DEFAULT '[]'` (hard errors blocking accept), `parse_warnings jsonb DEFAULT '[]'` (soft warnings advising but not blocking), and `raw_row jsonb NOT NULL` (original parsed row from the spreadsheet for diagnostic visibility). Indexes on `batch_id` and `scenario_id` (for staging-page lookup), `status` (for stale-batch sweeps in future cleanup). RLS: read gated by `tuition.view` + `can_view_family_details`; insert/update gated by `tuition.edit` (RPC paths perform their own checks; RLS is belt-and-suspenders). Both tables get the standard `tg_log_changes` trigger so INSERT/UPDATE/DELETE events are captured for audit. The `change_log_read` policy is rebuilt to include the two new tables. notify pgrst at end.
- **Migration 043**: Final Budget reference snapshot mechanism (v3.8.20). The architectural keystone for cross-module KPI wiring. Adds `tuition_worksheet_snapshots.is_final_budget_reference bool NOT NULL DEFAULT false` plus a partial unique index `tuition_worksheet_snapshots_one_final_budget_reference ON (scenario_id) WHERE is_final_budget_reference = true` enforcing the one-promoted-per-scenario invariant. New RPC `mark_snapshot_as_final_budget_reference(p_snapshot_id)` — SECURITY DEFINER; validates Stage 2 + caller has tuition.edit; atomically demotes any prior reference for the scenario then promotes the target (demote-then-promote ordering avoids the unique-index conflict). Bypasses `tg_prevent_snapshot_update` via `set local session_replication_role = replica` (same pattern as Migrations 031 and 038 backfills). Writes a synthetic change_log row pointed at the scenario (`field_name='__snapshot_promoted__'`, `reason='snapshot_promoted: <label>'`) for Recent Activity surfacing per v3.8.17 audit-trail-surfacing lesson. Returns the promoted snapshot_id. notify pgrst at end.
- **Migration 045**: Tuition Audit — Multi-Student Discount override storage (v3.8.21). Adds `tuition_worksheet_family_details.multi_student_discount_amount numeric(12,2) NULL` (mirrored on `tuition_worksheet_snapshot_family_details`). NULL = auto-compute; non-null = operator override. Second instance of the Override-capable projection pattern (Appendix C v3.8.20) after Faculty Discount in B2a (Migration 036). The math cascade decision (Q-cascade-1): Net Tuition Rate column display stays at the canonical tier-derived value when Multi-Student is overridden; Subtotal Tuition for Year computes as `base_rate × students_enrolled − effective_multi_student_discount` (override flows through). Faculty rows always render Multi-Student as `$0` and the cell is non-editable (faculty rule preserved); toggling `is_faculty_family` either direction clears any prior override (cascade contract — same pattern as the faculty_discount_amount cascade). Migration 045 also re-issues `capture_tuition_audit_snapshot` (CREATE OR REPLACE) to extend its `snapshot_family_details` INSERT with the new column — closing the M034 audit-discipline gap immediately for the snapshot-capture path. Function signature unchanged. notify pgrst at end.
- **Migration 044**: Tuition KPI reader RPCs (v3.8.20). Two read-only SECURITY DEFINER functions for the cross-module KPI flow. `get_latest_locked_tuition_planning(p_aye_id)` returns the locked Stage 1 snapshot's relevant fields (base_rate, total_students, total_families, breakeven_enrollment, faculty_discount_pct, curriculum_fee, ba_hourly_rate, captured_at, scenario_label, stage_display_name) — read by both Preliminary and Final Budget for "what we charge" data. `get_tuition_audit_final_budget_reference_summary(p_aye_id)` returns the promoted Audit snapshot's aggregates computed from `tuition_worksheet_snapshot_family_details`: `total_students = SUM(students_enrolled)`, `total_families = COUNT(*)`, `net_tuition_for_year = SUM(applied_tier_rate × students_enrolled − faculty − other − fa)`. Math mirrors `src/lib/tuitionMath.js`'s `computeFamilyNetTuition` per the v3.8.10 round-trip equivalence pattern. Both functions validate `tuition.view` before returning data; both return zero rows when their precondition isn't met (no Stage 1 lock / no promoted reference) so consumers can render "Pending …" KPI subtitles. Mirrors the Migration 030 `get_latest_locked_budget` pattern for the consumer side of cross-module data flow. notify pgrst at end.
- **Migration 042**: Tuition-B2-import polish — validation gap fixes (v3.8.19). Three validation rules from the B2-import build prompt didn't engage as expected in Migration 041; v3.8.19 closes the gaps via CREATE OR REPLACE on `tg_validate_tuition_import_row` and `create_tuition_audit_import_batch` (no schema change). **Rule 1**: negative discount values flip from hard error to warning (`'Discounts are entered as positive numbers; the system displays them as negative on the page.'`) — operator decides whether to proceed; the value stages as-parsed. **Rule 2**: unparseable dates (client-flagged) now surface in the staging UI. The upload modal previously dropped client-side parse outcomes when posting to the RPC — only normalized data crossed the wire. v3.8.19 has the upload modal include `client_parse_errors` and `client_parse_warnings` arrays per row in the payload, and the RPC reads those as the STARTING set of staged-row errors/warnings, APPENDING server-side validation outcomes via jsonb concatenation. Result: client-detected unparseable dates now block accept with the spec'd message (`'Date format not recognized. Use MM/DD/YYYY or YYYY-MM-DD.'`). **Rule 3**: `date_withdrawn` before `date_enrolled` flips from hard error to warning (`'Withdrawal date appears to be before the enrollment date.'`) — operationally there are edge cases where unusual date relationships are legitimate. Both client-side parser and server-side validator emit this warning so it surfaces regardless of which layer caught the dates. Function signatures unchanged; behavior contracts on valid-data paths preserved. notify pgrst at end. **Class-of-issue note** captured in v3.8.19 history: spec-vs-implementation drift on validation logic is a recurring failure mode where the "happy path" gets implemented and edge-case validators get skipped without surfacing in compile errors or smoke tests; future builds should explicitly verify each spec'd validation rule fires against intentional bad data before declaring the validation surface complete.
- **Migration 041**: Tuition-B2-import — staging RPCs (v3.8.18). Three SECURITY DEFINER functions implementing the bulk import workflow on the M040 staging tables. **`create_tuition_audit_import_batch(p_scenario_id, p_file_name, p_file_format, p_parsed_rows)`** validates the scenario is Stage 2 + drafting + caller has tuition.edit, inserts the batch header, walks the `p_parsed_rows` jsonb array inserting one staged row per element, runs server-side validation per-row via the helper `tg_validate_tuition_import_row` (hard errors: missing family_label, missing/invalid students_enrolled, negative discount amounts, date_withdrawn before date_enrolled; soft warnings: faculty_discount_amount on non-faculty row), populates parse_errors / parse_warnings, returns the new batch_id. **`accept_tuition_audit_import_batch(p_batch_id, p_mode)`** validates the batch is staged + caller has tuition.edit + no row has parse_errors (defense in depth), reads scenario configuration (base rate, top tier_size, faculty_discount_pct), in replace mode DELETEs existing family_details rows in the same transaction, walks staged rows in row_number order INSERTing into family_details with the faculty cascade applied (faculty rows → applied_tier_size=1, applied_tier_rate=base_rate, faculty_discount_amount auto-populated when not provided; non-faculty rows → tier resolves from students_enrolled capped at top tier_size), marks batch accepted with mode and accepted_at, writes a synthetic change_log row pointed at the scenario (`target_table='tuition_worksheet_scenarios'`, `field_name='__import_accepted__'`, `reason='import_accepted: <count> families <mode>'`) for Recent Activity surfacing per v3.8.17 audit-trail-surfacing lesson. Returns the committed row count. **`reject_tuition_audit_import_batch(p_batch_id, p_reason)`** validates batch is staged + caller has tuition.edit, marks batch rejected with rejected_at and rejected_reason; staged rows are RETAINED for audit (the table row stays queryable as a historical record). Writes a synthetic change_log row pointed at the scenario (`field_name='__import_rejected__'`). Returns the batch_id. All three RPCs are GRANT EXECUTE TO authenticated. notify pgrst at end.
- **Migration 039**: Tuition-B2-final-fixes — drop snapshot reason taxonomy, restore snapshot audit log (v3.8.17). Two bug-class fixes packaged together. **Taxonomy drop**: Migration 038's CHECK constraint `tuition_snapshots_reason_check` is dropped; the `snapshot_reason` column is preserved as nullable for back-compat with any test snapshots taken during the B2-final walkthrough. The `capture_tuition_audit_snapshot` RPC drops its `p_snapshot_reason` parameter — three-arg → two-arg `(p_scenario_id, p_snapshot_label)`. PostgreSQL parameter-list change requires DROP FUNCTION before CREATE (same gotcha as Migration 037); function body otherwise mirrors Migration 038's snapshot-capture logic with `snapshot_reason` writing NULL. **Recent Activity audit-log fix**: Migration 038's RPC inserted into `tuition_worksheet_snapshots` which fired `tg_log_changes` and created a `change_log` row, but with `target_table = 'tuition_worksheet_snapshots'` — Tuition-D's `MODULE_AUDIT_CONFIGS.tuition` only queries change_log rows with `target_table = 'tuition_worksheet_scenarios'`, so the snapshot event was invisible to the Recent Activity feed. Migration 039's RPC now writes a synthetic change_log row at the end of its body pointed at the scenario row: `target_table = 'tuition_worksheet_scenarios'`, `target_id = p_scenario_id`, `field_name = '__snapshot_captured__'` (parallel to system markers `__insert__` / `__delete__`), `new_value = jsonb_build_object('snapshot_id', ..., 'label', ..., 'captured_at', ...)`, `reason = 'snapshot_captured: <label>'`. The activity feed picks it up; `auditLog.js` classifyEvent + summarizeEvent gain a `'snapshot_captured'` kind to render it as "Snapshot captured: \"\<label\>\"" with a gold left rule. The vestigial `snapshot_reason` column is documented as deprecated-in-place; no code path writes it going forward. notify pgrst at end.
- **Migration 038**: Tuition Audit operator-triggered snapshot capability (v3.8.16). Codifies the architectural correction that **Tuition Audit (Stage 2) is not a lockable artifact** — it is a living working document maintained throughout the school year. Three new columns on `tuition_worksheet_snapshots`: `snapshot_reason text` (CHECK on the taxonomy `'lock' | 'midyear_reference' | 'fall_semester_end' | 'spring_semester_end' | 'school_year_end' | 'other'`; nullable for backward compat with pre-M038 lock snapshots), `snapshot_label text` (operator-provided label; nullable; required only when reason = `'other'`), `captured_at timestamptz NOT NULL DEFAULT now()` (the semantic capture time; equals `locked_at` for lock snapshots, equals `now()` at click time for operator snapshots). Existing snapshot rows backfilled to `snapshot_reason = 'lock'` and `captured_at = locked_at` (no-op against empty production tables; documents semantic intent). New RPC `capture_tuition_audit_snapshot(p_scenario_id, p_snapshot_reason, p_snapshot_label)` is the operator-triggered snapshot path — SECURITY DEFINER, requires `tuition.edit`, validates Stage 2 + drafting state + non-`'lock'` reason + label-required-when-`'other'`, copies every captured-by-value column on the scenario into a new snapshot row and every `family_details` row (including Migration 036's three columns) into `snapshot_family_details`. Returns the new snapshot id. Does NOT change scenario state. Closes the M036 known gap for the operator-snapshot path; `lock_tuition_scenario` is intentionally NOT modified (Stage 1 has no `family_details` rows so the M036 gap doesn't affect it). The RPC sets `app.change_reason = 'audit_snapshot_captured: <reason> (<label>)'` so the audit feed recognizes the snapshot event distinctly from edits and lock events. Application-side note: snapshot rows for operator captures use the sentinel `locked_via = 'snapshot'` to satisfy the M023 NOT NULL constraint inherited from the schema; the Snapshots Panel UI distinguishes lock vs. operator snapshots via `snapshot_reason`, not via `locked_via`. notify pgrst at end.
- **Migration 047**: Tuition v3.8.20 RPC repair (v3.8.24). Completes the repair of Migration 043's silent partial failure. Migration 043 (v3.8.20) defined three things in a single file — (a) ADD COLUMN `is_final_budget_reference`, (b) the partial unique index `tuition_worksheet_snapshots_one_final_budget_reference`, (c) the RPC `mark_snapshot_as_final_budget_reference` — and post-apply verification confirmed all three were missing. Migration 046 (v3.8.23) re-issued the column and partial unique index using `IF NOT EXISTS` patterns; M046's scope didn't include the RPC. Verification of `pg_proc` after M046 surfaced that the RPC was still absent — the M043 silent failure had taken out the function-create as well. M047 ships the RPC verbatim from the M043 specification (CREATE OR REPLACE pattern; idempotent against any live state). Function contract unchanged: SECURITY DEFINER, search_path = public, validates Stage 2 + caller has tuition.edit; atomically demotes any prior reference for the scenario then promotes the target (demote-then-promote ordering avoids the unique-index conflict that would arise from a swap); bypasses `tg_prevent_snapshot_update` via `set local session_replication_role = replica` (same pattern as Migrations 031, 038, and 043's original spec); writes a synthetic change_log row pointed at the scenario (`field_name='__snapshot_promoted__'`, `reason='snapshot_promoted: <label>'`) for Recent Activity surfacing per v3.8.17 audit-trail-surfacing lesson; returns the promoted snapshot id. notify pgrst at end. **Architectural significance**: this is the second migration in the M043 repair sequence, demonstrating that the v3.8.23 Appendix C commitment ("verify post-apply, not assumed correct from migration-tool success") works when followed step-by-step. Each repair migration was verified before the next was scoped — M046's ADD COLUMN verified, M046's partial unique index verified, then `pg_proc` checked for the RPC and found absent → M047 scoped. Without the post-apply verification habit, M047's RPC absence would have surfaced only when an operator clicked "Mark as Final Budget reference" and got a runtime error from PostgREST. After M047 applies, the M043 surface is fully restored — column + index + RPC all present in `tuition_worksheet_snapshots` and `pg_proc`.
- **Migration 046**: Tuition v3.8.20 schema repair (v3.8.23). Defensive repair for Migration 043's silent partial failure. M043 (v3.8.20) defined `ADD COLUMN is_final_budget_reference boolean NOT NULL DEFAULT false` on `tuition_worksheet_snapshots` plus a partial unique index `tuition_worksheet_snapshots_one_final_budget_reference ON (scenario_id) WHERE is_final_budget_reference = true`; both DDL statements are present and well-formed in M043's file (lines 45–54). Operational reality: the v3.8.20 burndown surfaced "column does not exist" errors at runtime when the snapshot-promotion RPC was invoked, and verification SELECTs against the live database confirmed both the column AND the partial unique index were absent — yet M043 reportedly applied successfully via the Supabase tooling with no error visible to the operator. The RPC body (`mark_snapshot_as_final_budget_reference`) DID land; the only visible failure was at RPC-invocation time. Failure mode is undiagnosable without server-side audit logs (candidate explanations include transaction rollback on a downstream non-DDL error, application-tooling bug that swallowed an error and reported success, or manual operator action between apply and verification — none verifiable post-hoc). **Migration 046's repair**: re-issues the column-add with `ADD COLUMN IF NOT EXISTS` and the partial unique index with `CREATE UNIQUE INDEX IF NOT EXISTS` so the migration is idempotent against any possible live state. If the schema is already correct, M046 is a no-op; if missing (the live state today), M046 adds them. M043 itself is left in the migration log unchanged as a historical record per the standard discipline ("never edit a committed migration; ship a follow-up"). The migration ends with `notify pgrst, 'reload schema'` (defensive against schema-cache miss; operator should manually re-issue if the ALTER TABLE event doesn't trigger an automatic refresh — same pattern as M045's PostgREST cache symptom). The file body documents both the M043 mystery and the verification SELECTs operators should run after applying — codifying the v3.8.23 Appendix C commitment. **Class-of-issue note** (codified in Appendix C this same commit): third instance of "migration reported success but partially failed" (M032 → M033/M034 stale column references; M037 RPC parameter naming; M043 → M046 silent DDL non-execution). The systemic response is post-apply schema-state verification — small operational cost, catches silent partial failures that would otherwise surface as runtime errors during user testing.
- **Migration 034**: Tuition RPC audit pass — fix `lock_tuition_scenario` (v3.8.12). Same root cause as Migration 033: Migration 032 rewrote `lock_tuition_scenario` based on Migration 024's pre-B1.1 baseline, losing all Migration 027/028/029 column updates from the snapshot INSERT. The first live Approve-and-lock attempt after Migration 033 raised "column 'other_discount_envelope' of relation 'tuition_worksheet_snapshots' does not exist." Rather than a third single-function hotfix, this migration audits **every** tuition PL/pgSQL function against the current schema. **Audit scope (8 functions)**: `compute_tuition_scenario_kpis` (clean, M033 fix verified intact), `lock_tuition_scenario` (BROKEN — comprehensive rewrite required), `submit_tuition_scenario_for_lock_review` (clean), `reject_tuition_scenario_lock` (clean), `request_tuition_scenario_unlock` (clean), `approve_tuition_scenario_unlock` (clean), `reject_tuition_scenario_unlock` (clean), `create_tuition_scenario_from_snapshot` (clean — escaped the M032 reset only because M032 didn't rewrite it). **Migration 034 ships a single CREATE OR REPLACE for `lock_tuition_scenario` only**; clean functions are left untouched per the don't-rewrite-working-code discipline. The new `lock_tuition_scenario` INSERT covers every captured-by-value column on the snapshot table organized by schema-evolution era: M023 base columns (with M027 renames applied), M027 additions (`projected_faculty_discount_amount`, `total_students`, `total_families`, `top_tier_avg_students_per_family`), M028 additions (`projected_b_a_hours`, `projected_multi_student_discount`), M029 additions (`expense_comparator_mode`, `expense_comparator_amount`, `expense_comparator_source_label`), M032 additions (`expense_comparator_amount_at_lock`, `expense_comparator_source_label_at_lock`, `kpi_breakeven_enrollment`, `kpi_net_education_program_ratio`). Note on naming: the snapshot has BOTH `expense_comparator_amount` (M029 mirror) AND `expense_comparator_amount_at_lock` (M032 addition); both columns exist and are nullable, so the fix populates both with the same value for backward compatibility — neither has ever been populated in production yet (no successful Tuition lock has happened). **Lesson captured**: future migrations that rewrite an existing PL/pgSQL function via CREATE OR REPLACE must read the CURRENT function body (e.g., from `pg_get_functiondef()` or the most-recent migration that touched it) rather than copying from an older migration. The audit habit established here is the safeguard until automated test coverage exists. No schema change. notify pgrst at end.

### Migrations needed (per this architecture)

- **Migration 047**: Strategic Plan schemas (three instruments)
- **Migration 048**: Snapshot tables for Staffing, Enrollment (Budget shipped in 011; Tuition shipped in 023)
- **Migration 049**: Board Composition + Committees
- **Migration 050**: Org Acronyms registry, Custom KPI registry
- **Migration 051**: Module-to-Account mappings
- **Migration 052**: Workflow Configuration UI scaffolding (Phase R2) — depends on the Settings UI work; may add a dedicated `workflow_config` module-permission row
...
| Tier Rates editability model: discount-driven per-student rates | Tier 1 (BASE) `per_student_rate` is the editable base input; tiers 2+ store an editable `discount_pct` and a read-only computed `per_student_rate = Math.round(base × (1 − discount_pct / 100))`. Editing the base cascades to all tier 2+ rates atomically; editing a tier discount recomputes only that tier. Two-decimal `discount_pct` precision matches school-side budgeting spreadsheets and round-trips dollar values cleanly. The application save path is the source of truth for `per_student_rate`; no DB validators added — adds trigger complexity without much safety. New tiers seed at 0% discount, requiring intentional entry. | 7.3 |
| Tuition Stage 1 lock_via is always 'cascade' or 'override' | Tuition has no upstream cascade requirements at lock time — the cascade chain runs Tuition→Budget, not the reverse. So Tuition's `locked_via` valid values are `'cascade'` (the normal "no upstream blocker" path; analogous to Budget's `'normal'` but named for symmetry with §7.5's vocabulary) and `'override'` (the admin bypass path with required justification). `lock_tuition_scenario` validates this enum at the function level (no DB CHECK; mirrors Budget's pattern from Migration 011). Never expand the enum without updating `LockReviewModal` and `TuitionLockBanner`'s rendering branches. | 7.3, 7.5 |
| Server-side KPI computation at lock time, client-side at drafting time (hybrid) | Stage 1 KPIs (Multi-Student Discount, Net Ed Program Ratio, Breakeven Enrollment, etc.) compute client-side via `src/lib/tuitionMath.js` during drafting for instant feedback as the user iterates tier rates / breakdowns / discounts. At lock time, `compute_tuition_scenario_kpis` recomputes the same KPIs server-side and the values are captured into the snapshot — single transactional capture so locked KPIs cannot drift from locked configuration even if `tuitionMath.js` evolves. The two implementations must stay in sync; the discipline is to land the SQL change in the same migration as the JS change when the formulas evolve. Tuition-D (Migration 032) extended both layers in lockstep for the ratio + breakeven KPIs. | 7.3 |
| Cross-module cascade enforcement is prospective, not retroactive | Cross-module cascade rules (§7.5) fire on lock submissions made AFTER the rule ships in code. Historical artifacts locked before the rule's introduction are preserved as-is — they predate the rule. AYE 2026's Preliminary Budget was locked-with-override before the Tuition→Budget cascade was wired up at all and remains locked under that override; the Tuition Stage 1 cascade fires only on the next lock cycle (after an unlock + re-lock, or AYE 2027 reaches Preliminary Budget). Same prospective-enforcement principle that applied to Migration 015's sibling-lock guard at its introduction. The alternative — retroactively invalidating locked artifacts — would compromise the snapshot binding rule (§5.1) and violate the trust model that locked artifacts are immutable governance documents. | 7.5 |
| Synthetic test user as documented fixture | `treasurer.test@libertas.local` is the canonical second-identity for two-identity workflow testing. Permissions extend with each new module that ships with two-identity workflow (Budget, Tuition today; Staffing, Enrollment Estimator, Final Budget, Tuition Audit Stage 2 as those modules ship). Documented in §9.8 with the maintenance discipline. The hierarchical `permission_level` enum means granting `approve_unlock` subsumes edit; the testing convention "do not use the synthetic user for data entry" is enforced socially rather than by schema. | 9.8 |
| Faculty discount replaces multi-student tier discount (operational rule) | Faculty families pay base_rate × students × (1 − faculty_discount_pct/100). The multi-student tier discount does NOT apply to faculty — the two discounts do not stack. Implementation: `is_faculty_family` flag on family rows triggers a save-path cascade — `applied_tier_size = 1`, `applied_tier_rate = base_rate`, `faculty_discount_amount` auto-populates from base × students × pct. Storage is Option B (computed value persists to the `faculty_discount_amount` column on cascade events). The Multi-Student Discount column **defaults** to $0 for faculty families. Manual overrides (faculty_discount_amount, applied_tier_rate, **multi_student_discount_amount as of v3.8.22**) are supported for non-standard cases (legacy spreadsheet's Cookson example: faculty discount applied AFTER multi-student tier discount due to FA / contract signing timing) and visually marked with a small gold dot at the cell's top-right corner with a hover tooltip showing the auto-computed value. Override detection runs at render time; "override" means stored value ≠ auto-computed value. **v3.8.22 amendment**: the faculty rule's $0 Multi-Student is a default, not a hard block — operators can override directly on faculty rows for Cookson-class cases rather than working around it via Faculty Discount composition. | 7.3 |
| Stage 2 per-family editor mirrors legacy Google Sheet structure | The Stage 2 audit editor (TuitionFamilyDetailsTable) carries forward the column architecture of Libertas's legacy AYE 2026 audit Google Sheet for continuity with the operator's existing mental model: 11 numeric / text columns (Family Name, # Enrolled, Base Tuition, Multi-Student Discount, Net Tuition Rate, Subtotal Tuition for Year, Faculty Discount, Other Discounts, Financial Aid, NET Tuition for YEAR, Notes), plus an `is_faculty_family` toggle in a narrow leading column, plus `date_enrolled` and `date_withdrawn` columns for mid-year enrollment tracking. Computed columns mirror the spreadsheet's formulas; faculty families' Multi-Student Discount column **defaults** to $0 (the rule does not apply by default; v3.8.22 made the cell editable so operators can override the default for Cookson-class one-off cases); the NET Tuition for YEAR is the bottom-line per-family annual amount the family will pay. The per-row Notes column is the audit-trail-grade governance annotation field (FA committee actions, board decisions, override justifications); a small clock affordance opens a per-row notes-history modal. | 7.3 |
| School year vs AYE temporal distinction | Three temporal concepts coexist in Agora and must not be confused: the AYE / fiscal year (7/1 → 6/30; planning, contracting, accounting container; defined in `academic_years` schema), the school year / academic calendar (when students physically attend; varies annually; defined by the school's calendar; for Libertas approximately Sept 8 → May 29; will be configured in a future Governance Calendar module per Phase 8), and per-family enrollment dates (`date_enrolled` / `date_withdrawn` on `tuition_worksheet_family_details` rows; operator-entered raw dates with null defaults; no system-side school-year inference today). Stage 2 audit semantics depend on actual school-year enrollment, not fiscal-year boundaries. Pro-ration of mid-year enrollment is operator-handled via the discount fields and Notes context. When the Governance Calendar module ships, per-family date pickers can default from the configured school-year start date and the audit can surface "enrolled before school year start" / "withdrew before school year end" warnings. | 3.3, 7.3 |
| Tuition Audit (Stage 2) is a working document, not a lockable artifact | Stage 2 is a living working document maintained throughout the academic year — families are added during summer pre-enrollment, mid-year transfers happen, families withdraw, financial aid committee decisions are recorded as they occur. There is no single moment at which "the audit is done and gets locked"; the audit IS the running record. The Stage 2 scenario stays in `state = 'drafting'` indefinitely. There is no submit-for-lock-review, no approve-and-lock, no two-identity unlock cycle. This corrects the earlier roadmap (B2b on the build sequence) which had a Stage 1-parallel lock workflow planned for Stage 2 — that plan was based on a misreading of operational reality. | 7.3 |
| Operator-triggered snapshots replace lock workflow for Stage 2 | Reference points within a Stage 2 scenario are captured via operator-triggered snapshots, not via lock events. The Capture Snapshot affordance in the page header opens a modal with a single freeform `snapshot_label` text field. The `capture_tuition_audit_snapshot` RPC (Migration 039 — two-arg signature) creates an immutable snapshot of the current scenario state including all `family_details` rows; multiple snapshots may exist per scenario throughout the year. Snapshots are read-only reference points (no second-identity approval required for capture — they document state, not governance decisions). **Amendment (v3.8.17)**: Migration 038 originally introduced a fixed snapshot reason taxonomy via a CHECK constraint (`'midyear_reference'` / `'fall_semester_end'` / `'spring_semester_end'` / `'school_year_end'` / `'other'`). The B2-final walkthrough surfaced that schools have varying operational calendars and the taxonomy added rigidity without analytical value — the freeform `snapshot_label` already conveys what the operator wants to communicate about a snapshot's purpose. Migration 039 dropped the CHECK constraint and the `p_snapshot_reason` RPC parameter; the `snapshot_reason` column is preserved as nullable vestigial (deprecated-in-place) for back-compat with test snapshots taken between Migrations 038 and 039. | 7.3 |
| Final Budget cascade depends on Tuition Stage 1 lock only | Final Budget reads tuition revenue projections from the locked Stage 1 snapshot; Stage 2's running-actuals data is the source for variance reporting against Stage 1 projections, not a Final Budget input. So the previous architectural assumption "Final Budget cascade requires Stage 2 locked" is retired (and was never enforced in code — it had been documented in §7.5 but no application-layer check ever fired on it). Final Budget continues to gate on the Tuition module instance state being `'locked'` via the existing `school_lock_cascade_rules` row; under the v3.8.16 model, Stage 1 lock is what transitions the tuition module instance to `'locked'`. | 7.5 |
| Tuition import uses XLSX template with embedded Instructions tab; accepts both XLSX and CSV on upload | The downloaded template is a multi-tab XLSX (`Instructions` tab with column descriptions, important rules, and live reference values pulled from the active scenario; `Family Data` tab with formatted column headers). XLSX format chosen over CSV-only for the template because the Instructions tab cannot exist in CSV — the embedded reference is the principal usability win, allowing operators to fill out the file without holding the Agora page in mind. Upload accepts BOTH XLSX and CSV (operators may save-as CSV for cross-app compatibility); SheetJS reads both formats via the same `XLSX.read` entry point. Filename convention: `Libertas_Agora_{AYE label}_Tuition_Audit_Import_Template.xlsx`. SheetJS library installed from the SheetJS CDN tarball (`xlsx-0.20.3.tgz`) rather than the npm-published `xlsx` package, which has known CVEs with no fix available via npm. | 7.3 |
| Import mode (append vs replace) chosen at acceptance time, not template time | The append-vs-replace decision is a destructive-action gate — replace mode permanently deletes existing family_details rows. Putting the choice at template / upload time would require operators to make the decision before seeing the staged data; putting it at acceptance time means they review the staged rows in context (envelope tracker preview shows post-accept envelope state for both modes) and only choose mode after that review. Replace mode shows an additional confirmation modal naming both row counts. The decision-at-acceptance-time pattern parallels Budget's submit-for-lock-review pattern: deliberate confirmation pauses for governance-weight actions. | 7.3 |
| Server-side validation in the import RPC; client-side validation for UX only | Defense-in-depth pattern. The client-side parser (`tuitionImportParser.js`) normalizes display column names → snake_case schema columns and produces a per-row package with `errors` and `warnings` arrays for the upload preview. The SECURITY DEFINER RPC `create_tuition_audit_import_batch` re-validates EVERY row server-side via the helper `tg_validate_tuition_import_row` and writes the validation outcome to the staged row's `parse_errors` / `parse_warnings` jsonb columns. The client is the trust-only-the-UX layer; the database is the trust boundary. The same posture applies to the `accept_tuition_audit_import_batch` RPC, which re-validates that no staged row has parse_errors before committing (defense against direct RPC calls that bypass the staging UI). | 7.3 |
| Final Budget anchors to operator-promoted Audit snapshot, not live Audit data | Final Budget reads tuition data from a specific Audit snapshot the operator explicitly promoted as "the Final Budget reference" (Migration 043 + 044), not from live Audit state. Operationally: schools build Final Budget against a known enrollment baseline — typically the early-fall snapshot once enrollment finalizes — and Final Budget should not move targets every time a mid-year audit edit happens. The reference-snapshot indirection separates "what we plan to spend" (Final Budget construction) from "what we are seeing happen" (live Audit). One snapshot per scenario at a time can be the reference; promotion is atomic (demote prior + promote new in one transaction); changing the reference is an explicit operator action recorded in the activity feed, not a side effect of any edit. The promoted snapshot's data flows to Final Budget via `get_tuition_audit_final_budget_reference_summary` RPC. | 7.3 |
| Cross-module KPI consumers compute client-side from cached reads | The `KpiSidebar` receives upstream Tuition refs (`tuitionStage1` from `get_latest_locked_tuition_planning`; `tuitionAuditRef` from `get_tuition_audit_final_budget_reference_summary`) as React props loaded once on AYE / stage change in `BudgetStage`. Cross-module KPIs (Cost per Student, Tuition Gap, Break-even Enrollment) compute client-side from those cached refs plus the live `kpis.totalExpense`. As the operator edits Final Budget expense lines, the dependent KPIs recompute live without further server round-trips; the upstream Tuition data stays cached and frozen until page reload. This pattern matches the v3.8.7 client-side hybrid for Tuition Stage 1 KPIs and avoids the round-trip cost of recomputing on every keystroke. The trade-off: changing the upstream reference (e.g., re-promoting a different Audit snapshot) requires a page reload to pick up; acceptable because re-promotion is a rare deliberate action. | 7.3 |
| Adding a column to a render-state table requires lockstep updates across migration + write path + read SELECT | When a column is added to a table that participates in render state (i.e., the column is read on page mount and round-tripped through user edits), three places need updating in the same commit: (1) the migration that creates the column on both base and snapshot tables; (2) the write path / patch handler that allows the column to be saved; (3) the data-load SELECT that pulls the column back on page load. Missing any one creates a partial-failure mode that's deceptively easy to ship: the migration applies, the cell renders, the save fires successfully, the optimistic local state shows the saved value — but on page refresh the load SELECT omits the column, the family object has `undefined` for the field, render falls to the auto-compute / null-default branch, and the operator perceives the value as "reverted." The save was correct; the load was incomplete. The v3.8.25 instance: M045 added `multi_student_discount_amount` to `tuition_worksheet_family_details`, the cell was wired editable in TuitionFamilyDetailsTable, the save handler in handleUpdateRow forwarded the patch correctly — but the loadFamilies SELECT in TuitionAuditPage.jsx kept the pre-M045 column list. Symptom only manifested on refresh, which is why v3.8.21 / v3.8.22 walkthroughs (no refresh between save and observation) didn't catch it. The same omission existed on a secondary read path in TuitionAuditImportStaging.jsx's envelope-preview SELECT. **Lockstep checklist for column adds**: when shipping a migration that adds a render-state column, grep for every SELECT against the affected table (`grep -rn "from\\|FROM .*<table>"`) and confirm each SELECT either includes the new column or is intentionally excluded. The cost is small; the benefit is closing a class of bugs that surface only on refresh and are easy to misread as "the save isn't persisting." | 5.1, 7.3 |
| Migrations must be verified post-apply, not assumed correct from migration-tool success | Migration tooling reporting "applied successfully" is necessary but not sufficient evidence that the migration's intended schema changes actually landed in the live database. Three known instances of "migration reported success but partially failed" in the Tuition module's evolution: (1) **M032 → M033/M034** — `compute_tuition_scenario_kpis` and `lock_tuition_scenario` were rewritten from M024's pre-B1.1 baseline and lost M027's column renames; the function-create succeeded but every invocation raised at runtime ("record 'v_scenario' has no field 'other_discount_envelope'"). (2) **M037** — `create_tuition_scenario_from_snapshot` was created with parameter name `p_scenario_label` while the application call site used `p_scenario_name`; the function existed but was unreachable via PostgREST's named-parameter resolution. (3) **M043 → M046** — DDL statements (ADD COLUMN + partial unique index) didn't execute against the live database despite the migration file being well-formed and the migration tooling reporting success; failure surfaced only when the snapshot-promotion RPC tried to UPDATE the absent column at runtime. The systemic response: after applying any migration that touches tables, indexes, or function signatures, run schema-state verification SELECTs (`information_schema.columns`, `pg_indexes`, `pg_get_functiondef`) confirming the actual state matches the migration's intent. The v3.8.12 audit-discipline lesson — read the CURRENT function body before rewriting — is the *pre-apply* complement; this *post-apply* commitment closes the loop. Cost: small (a handful of SELECTs). Benefit: catches silent partial failures that would otherwise surface as runtime errors during user testing, which is exactly what happened in each of the three instances above. M046's file body documents the pattern in operational terms — verification SELECTs are commented in at the end of the migration as the operator's post-apply checklist. **Evidence the discipline works** (v3.8.24 update): the M043 repair sequence ran two iterations precisely because verification was performed at each step. M046 (v3.8.23) repaired column + partial unique index; post-apply SELECTs confirmed both landed. The same session then ran a `pg_proc` check for `mark_snapshot_as_final_budget_reference` and found the RPC was ALSO absent (M043 had silently failed in entirety, not just on the DDL statements). M047 (v3.8.24) was scoped from that finding and shipped as a second repair migration. Without the post-apply habit, the missing RPC would have surfaced only when an operator clicked the snapshot-promotion button and got a PostgREST runtime error — exactly the symptom the discipline is designed to prevent. The cost-benefit case is now empirically demonstrated: ~30 seconds of SQL SELECTs per migration apply caught a real second-order failure mid-flight in the M043 repair work. | Appendix B |
| Override-capable projection pattern | Default values flowing from upstream sources should be operator-overridable, with override state detectable and visually indicated. Examples already shipped: faculty_discount_amount auto-populates on the family detail row and accepts override (gold-dot indicator per Appendix C v3.8.14); **multi_student_discount_amount on the family detail row (v3.8.21, Migration 045)** — second instance of the pattern, applied to the Multi-Student Discount column with the same NULL-means-auto, non-null-means-override semantics; Stage 1 `total_students` will, when the Enrollment Estimator module ships, default from the locked Enrollment Estimator snapshot but retain manual override capability for the Tuition Committee. Generalized commitment for future modules: cross-module data flows that produce a single value (a count, a rate, a target) should use a stored-with-override-detection pattern (Option B per the M027 / B2a / v3.8.21 pattern — store the computed value when overridden; render the auto value when not stored; detect divergence at render time by comparing stored vs. fresh-computed; surface divergence visually with the gold-dot indicator). The pattern preserves operator agency over upstream defaults without losing the ability to recompute defaults when source data changes. **Cascade contract** (v3.8.21 codification): when the upstream toggle that produces the auto value changes (e.g., is_faculty_family flip on the family row), any stored override should be cleared — the operator's prior override against the old auto-context isn't meaningful in the new context, and stale overrides invite confusion. **Rule-as-default principle** (v3.8.22 codification): operational rules expressed in the auto-value computation (e.g., "faculty rule sets Multi-Student to $0") are DEFAULTS, not hard blocks on the override capability. The override path applies uniformly across rule-affected and rule-unaffected rows; the rule preserves standard semantics on the auto path while the override path remains universally available for one-off cases (Cookson-class). The earlier v3.8.21 implementation that hard-blocked override on faculty rows was reverted in v3.8.22 after real-data discovery surfaced that the block forced operators into compositional workarounds (mark family non-faculty + Faculty Discount line entry) that didn't match their mental model. | 3.6, 7.3 |
...
- **v3.8.9** — May 3, 2026 — Tuition-B1.6: Tier Rates editability flip — discount-driven per-student rates. Real-data walkthrough on Scenario 2 surfaced that the prior model (all per-student rates editable, discount column read-only) required users to manually compute tier rates with a calculator while toggling between scenarios — exactly the friction the tool exists to eliminate. The new model matches the legacy Google Sheet's tier-rate input pattern. (1) **Editability flip**: Tier 1 (BASE) `per_student_rate` stays editable as the base input; the DISCOUNT column cell renders empty (the BASE badge in the Tier column conveys the meaning). Tiers 2+ have `discount_pct` as the editable input (numeric, 2-decimal precision; "%" suffix on display); `per_student_rate` becomes read-only computed value rendered without input chrome. (2) **Cascade**: editing the base rate atomically recomputes all tier 2+ `per_student_rate` values from stored `discount_pct` and writes the entire `tier_rates` jsonb in one save — the user types a new base rate, tabs away, and sees all tier 2+ cells update simultaneously. Editing a single tier's `discount_pct` recomputes only that tier's `per_student_rate`. The cascade flows through the existing B1.2 `persistFields` recompute of `projected_multi_student_discount` automatically — no page-level changes needed. (3) **New tiers** seed at `discount_pct = 0%` with `per_student_rate = base_rate`, requiring the user to enter a real discount intentionally. (4) **Migration 031**: `tier_rates` jsonb shape change. Each row gains a `discount_pct` field (numeric, 2-decimal precision via application-side rounding). Backfill via PL/pgSQL DO block: tier 1 sets `discount_pct = 0` unconditionally; tiers 2+ compute from `ROUND(((tier_1_rate - this_tier_rate) / tier_1_rate) × 100, 2)`. Per-student rate values preserved verbatim — no rounding, no recomputation. Round-trip math preserves dollar values exactly: AYE 2026 walkthrough scenario at $6,720 / $6,615 / $6,300 / $5,775 → 0% / 1.56% / 6.25% / 14.06% → recomputed back to identical dollar values. Snapshot mirror identical (uses `set local session_replication_role = replica` to bypass the snapshot-immutability trigger for the backfill). (5) **No new DB validators**: the application save path is the source of truth for `per_student_rate` (computes atomically from `discount_pct`); DB-level "tier 1 must = 0" or "discount must be < 100" adds trigger complexity without much safety. The existing `tg_validate_tuition_scenario` from Migration 027 (breakdown_pct sum check) stays as-is. (6) **`tuitionMath.js` extension**: new `computePerStudentRateFromDiscount(baseRate, discountPct)` helper — `Math.round(baseRate × (1 − discountPct/100))` with strict null propagation. The existing `tierDiscountPct(tierRate, tier1Rate)` stays for backward compat with downstream readers that haven't migrated to reading the stored `discount_pct` field. (7) **`tuitionDefaults.js` extension**: `defaultTierRates()` seed gains `discount_pct: 0` per row so fresh scenarios start with valid jsonb shape. (8) **No SECURITY DEFINER function changes**: `lock_tuition_scenario` and `create_tuition_scenario_from_snapshot` write/read the `tier_rates` jsonb opaquely; the new field carries through. (9) **Architecture doc**: §7.3 schema block extended; new "Tier Rates editability model" subsection codifies the cascade, the new-tier seeding rule, the round-trip math identity, and the workflow rationale (legacy spreadsheet alignment). Appendix B Migration 031 entry; needed migrations renumbered 032-037. Appendix C: new "Tier Rates editability model: discount-driven per-student rates" decision row. Document header bumped 3.8.8 → 3.8.9.
- **v3.8.25** — May 5, 2026 — Tuition-AuditSavePersistence: per-family editor's Multi-Student Discount override now round-trips correctly on refresh. The operator reported across multiple sessions that values entered into editable cells on the Tuition Audit page reverted to prior state on page refresh. **Diagnosis from the codebase** (per the v3.8.25 prompt's investigation plan): traced the save path from cell `onBlur` → `onSave` callback → `handleFieldSave` → `onUpdateRow` prop → `handleUpdateRow` in `TuitionAuditPage.jsx` → `supabase.from('tuition_worksheet_family_details').update({ ...patch, updated_by }).eq('id', familyId)`. The save path is correct: any column in `patch` is forwarded to Supabase, error responses are awaited and surfaced via toast, optimistic local state is rolled back on failure. So writes succeed for any column on the patch. **Mode H** (per the prompt's enumeration) — the `loadFamilies` SELECT at `TuitionAuditPage.jsx:266` omitted `multi_student_discount_amount`, the column added by Migration 045 in v3.8.21. v3.8.21's commit added the column to the migration, the table cell rendering, and the save handler — but missed updating the SELECT used by `loadFamilies`. Result: writes for the M045 override succeeded silently (the optimistic local update made the value visible on the page), but on refresh the column never came back, the family object had `undefined` for the field, `computeFamilyMultiStudentDiscount` fell to auto-compute, and the operator perceived the value as "reverted." Other columns (`faculty_discount_amount`, `other_discount_amount`, `financial_aid_amount`, `notes`, `is_faculty_family`, dates) are present in the SELECT and round-trip correctly — the reverting symptom was specific to Multi-Student override but easy to read as broader because the column is the one most operators were testing during the v3.8.21 / v3.8.22 walkthroughs. **Fix**: added `multi_student_discount_amount` to the SELECT in `loadFamilies` (TuitionAuditPage.jsx) and to the secondary envelope-preview SELECT in `TuitionAuditImportStaging.jsx` (which feeds `computeEnvelopesUsed` for the import-mode preview — same Mode H scoped to that surface). Both fixes are single-line column additions; no schema change, no migration, no save-path change. **Class-of-issue lesson**: when adding a render-state column, three places need updating in lockstep — migration + write path + data-load SELECT. Missing the SELECT creates a partial-failure mode that surfaces only on refresh, where the save appears to work and the cell shows the saved value via optimistic local state but the persisted value never makes it back to the rendered family object. This is harder to spot during in-session walkthroughs (no refresh between save and observation) than persistent failure modes. **Architecture commitment** (new Appendix C decision row): "Adding a column to a render-state table requires lockstep updates across migration + write path + read SELECT." Lockstep checklist: when shipping a migration that adds a render-state column, grep every SELECT against the affected table and confirm coverage. Document header bumped 3.8.24 → 3.8.25. **Diagnostic-process note**: this commit followed the v3.8.25 prompt's "investigate from the code side first" directive — read the page component, traced the save chain, identified the SELECT omission, fixed it. ~10 minutes start to finish, no browser-side network tracing needed. The codebase is its own diagnostic surface; reading it carefully beats interpreting opaque runtime symptoms.
- **v3.8.24** — May 5, 2026 — Tuition-RpcRepair: Migration 047 completes the M043 repair by restoring the missing `mark_snapshot_as_final_budget_reference` RPC. After M046 (v3.8.23) repaired the column + partial unique index, post-apply verification per the new Appendix C commitment surfaced that M043's silent failure had ALSO taken out the RPC — `pg_proc` had no row matching the function name. M047 ships the RPC verbatim from M043's original specification (CREATE OR REPLACE; idempotent against any live state). Function contract unchanged: SECURITY DEFINER; search_path=public; validates Stage 2 + caller has tuition.edit; demote-then-promote with `session_replication_role = replica` bypass of `tg_prevent_snapshot_update`; synthetic change_log row pointed at the scenario for Recent Activity surfacing; returns promoted snapshot id. After applying M047 + verifying via `SELECT proname, pg_get_function_arguments(oid) FROM pg_proc WHERE proname = 'mark_snapshot_as_final_budget_reference'`, the M043 surface is fully restored — column + index + RPC all present. **Architectural significance**: this is the second migration in the M043 repair sequence and demonstrates the v3.8.23 Appendix C commitment paying off in practice. Each repair migration was verified before the next was scoped: M046's column verified, M046's index verified, then `pg_proc` checked for the RPC and found absent → M047 scoped. The discipline caught a real second-order failure mid-flight rather than letting it surface as a runtime error during user testing. Appendix C decision row "Migrations must be verified post-apply" extended with the M046 → M047 evidence. Appendix B Migration 047 entry. v3.8.24 history entry. Document header bumped 3.8.23 → 3.8.24. No code changes; schema repair only. Future M043-derivative repairs are unlikely now that the full M043 surface is verified present in the live database.
- **v3.8.23** — May 5, 2026 — Tuition-SchemaRepair: Migration 046 repairs M043's silent partial failure + post-apply verification commitment. The v3.8.20 burndown surfaced "column does not exist" errors at runtime when the snapshot-promotion RPC was invoked. Verification SELECTs against the live database confirmed both the `is_final_budget_reference` column and the `tuition_worksheet_snapshots_one_final_budget_reference` partial unique index were absent on `tuition_worksheet_snapshots` — yet Migration 043 had reportedly applied successfully via the Supabase tooling with no error visible to the operator. The M043 file itself is well-formed (the column-add and partial unique index are present at lines 45–54); the failure mode is undiagnosable without server-side audit logs. (1) **Migration 046 — defensive repair**: re-issues the column-add with `ADD COLUMN IF NOT EXISTS` and the partial unique index with `CREATE UNIQUE INDEX IF NOT EXISTS` so the migration is idempotent against any possible live state. M043 itself is left in the migration log unchanged as a historical record per the standard "never edit a committed migration; ship a follow-up" discipline. M046's file body documents the M043 mystery and the verification SELECTs operators should run after applying — codifying the new architectural commitment in operational terms. Ends with `notify pgrst, 'reload schema'` (defensive against schema-cache miss). (2) **Architecture commitment** (new Appendix C decision row): "Migrations must be verified post-apply, not assumed correct from migration-tool success." Cites the three known instances of silent partial failure in the Tuition module's evolution — M032 → M033/M034 (stale column references in PL/pgSQL function rewrites), M037 (RPC parameter naming alignment after the file was committed), M043 → M046 (silent DDL non-execution) — and codifies the systemic response: after applying any migration that touches tables, indexes, or function signatures, run schema-state verification SELECTs (`information_schema.columns`, `pg_indexes`, `pg_get_functiondef`) confirming the actual state matches intent. The v3.8.12 audit-discipline lesson — read the CURRENT function body before rewriting — is the pre-apply complement; this post-apply commitment closes the loop. Class-of-issue framing: catch-the-class beats diagnose-the-instance when the instance's root cause is not diagnosable post-hoc. (3) **Architecture doc**: Appendix B Migration 046 entry; "Migrations needed" list renumbered (Strategic Plan schemas → 047, etc.); Appendix C new "Migrations must be verified post-apply" decision row; v3.8.23 history entry; document header bumped 3.8.22 → 3.8.23. (4) **No code changes** — the repair is schema-only; the v3.8.21 / v3.8.22 application-layer work that touched the family editor remains untouched. After applying M046, the operator should run the two verification SELECTs documented in M046's footer and confirm both return one row before resuming the v3.8.20 burndown.
- **v3.8.22** — May 5, 2026 — Tuition-AuditHotfix: faculty rows can now override Multi-Student Discount. Real-data discovery during the v3.8.21 walkthrough surfaced that the v3.8.21 design — which restricted Multi-Student Discount editability to non-faculty rows — was wrong. Cookson-class one-off cases (faculty family with a pre-signed Tuition Agreement that honored the multi-student tier discount before faculty status changed) require a direct override on the faculty row to align with the operator's mental model; the v3.8.21 compositional workaround (mark family non-faculty + Faculty Discount line entry) didn't match how operators think about these cases. v3.8.22 promotes the faculty rule from a hard block on Multi-Student override to a DEFAULT — the auto value for faculty stays at $0 (rule preserved on the auto path) but the cell is now editable on faculty rows just as it is on non-faculty rows. Override detection works uniformly across both row types: any non-zero stored value on a faculty row diverges from the $0 auto value and surfaces the gold-dot indicator. (1) **`src/lib/tuitionMath.js`**: `computeFamilyMultiStudentDiscount` reordered — the stored-override branch (`multi_student_discount_amount != null`) now runs BEFORE the faculty check, so faculty rows return the override when one is stored. `isFamilyMultiStudentOverridden` no longer short-circuits on faculty rows; the override-vs-auto comparison runs uniformly. `getFamilyMultiStudentAutoValue` unchanged — still returns 0 for faculty rows (the rule's default). `sumMultiStudentDiscountUsed` comment updated to reflect that faculty rows can now contribute non-zero overrides to the envelope total. (2) **`src/components/tuition/TuitionFamilyDetailsTable.jsx`**: removed the `family.is_faculty_family ? <ComputedCell> : <CurrencyCell>` branch in the Multi-Student Discount cell — always renders the editable CurrencyCell with the gold-dot wrapper. The `handleToggleFaculty` cascade still clears any prior `multi_student_discount_amount` override on either toggle direction (the auto-value context fundamentally changes on toggle — from `(base − applied) × students` to `0` or vice versa — so a stale override no longer reflects operator intent regardless of which direction). Cell-rendering comment updated to document the rule-as-default semantics. (3) **No migration**: the v3.8.21 schema column already supports the value on faculty rows. v3.8.22 is a pure application-layer relaxation. The snapshot capture path inherited from Migration 045 already includes `multi_student_discount_amount` so faculty-row overrides flow through to snapshots correctly. (4) **Architecture doc**: §7.3 "Faculty discount rule" paragraph updated (Multi-Student "displays $0" → "defaults to $0; cell remains editable per v3.8.22"). §7.3 "Multi-Student Discount override (v3.8.21)" subsection gains a v3.8.22 amendment paragraph documenting the rule-as-default promotion. Appendix C decision rows updated: "Faculty discount replaces multi-student tier discount" notes the v3.8.22 amendment; "Stage 2 per-family editor mirrors legacy Google Sheet structure" updates the faculty Multi-Student description from "displays $0" to "defaults to $0; v3.8.22 made the cell editable"; "Override-capable projection pattern" gains a "Rule-as-default principle (v3.8.22 codification)" addendum capturing the generalized lesson — operational rules expressed in auto-value computations are defaults, not hard blocks on override capability. Document header bumped 3.8.21 → 3.8.22. Class-of-issue note: when implementing Override-capable projection patterns, prefer permissive rendering of the override affordance (let operators override universally) over rule-derived restrictions (hard-blocking override on rows where the rule "doesn't apply"); the rule lives on the auto path, not on the override path. Architecture is restrictive only when there is a positive harm-of-override argument; absent that argument, the override should be available wherever the column exists.
- **v3.8.21** — May 5, 2026 — Tuition-AuditPolish: viewport-responsive table height + editable Multi-Student Discount with override pattern. Two refinements surfaced from real-data work. (1) **Multi-Student Discount column is now editable on non-faculty rows** per the Override-capable projection pattern (Appendix C v3.8.20) — second instance of the pattern after Faculty Discount in B2a (Migration 036). Migration 045 adds `multi_student_discount_amount numeric(12,2) NULL` to `tuition_worksheet_family_details` (mirrored on the snapshot table); NULL = auto-compute, non-null = operator override. The faculty rule is preserved unchanged: faculty rows render `$0` for Multi-Student Discount and the cell is non-editable. Toggling `is_faculty_family` either direction clears any prior multi-student override (cascade contract — same pattern as the faculty_discount_amount cascade). The math cascade decision (Q-cascade-1): Net Tuition Rate display stays at the canonical tier-derived value when Multi-Student is overridden; Subtotal computes as `base × students − effective_multi_student_discount` so the override flows into Subtotal and NET / YEAR. The math display has a subtle inconsistency in the override case (Net Tuition Rate × Enrolled ≠ Subtotal); that's intentional — the divergence is the override's signal. The Cookson-class one-offs (faculty family with pre-signed Tuition Agreement before faculty hire) compose existing affordances (Faculty Discount override on a faculty row, OR non-faculty row with auto Multi-Student plus a separate Faculty Discount entry); no special-case import rules are needed. Migration 045 also re-issues `capture_tuition_audit_snapshot` (CREATE OR REPLACE) to extend its `snapshot_family_details` INSERT with the new column — closing the M034 audit-discipline gap immediately for the snapshot-capture path. (2) **Viewport-responsive family table max-height**: the prior fixed `max-h-[calc(100vh-22rem)]` is replaced with a header-aware computation. Expanded reservation `~24rem` accommodates page header + 3-card grid + action bar; collapsed reservation `~13rem` reclaims the ~280-300px the card grid was using. `min-h-[300px]` floor prevents collapse below ~5 rows on small viewports. Sticky header preserved. On a 1920×1200 screen, ~25-35 rows visible expanded; on a 1366×768 laptop, ~8-12 rows after chrome. (3) **`tuitionMath.js` extensions**: `getFamilyMultiStudentAutoValue` (auto-only, ignores stored override; used for the gold-dot tooltip's "auto value: $X" text), `isFamilyMultiStudentOverridden` (drives the gold dot), and the existing `computeFamilyMultiStudentDiscount` extended to return the stored override when non-null on non-faculty rows. `computeFamilyNetTuition` updated to use `base × students − effective_multi_student` for subtotal — equivalent to the prior `applied_tier_rate × students` formulation when not overridden, but flows the override through to NET when overridden. (4) **Architecture doc**: §7.3 new "Multi-Student Discount override (v3.8.21)" subsection codifying the editability rules, the cascade decision, and the Cookson-class composition path. §7.3 new "Viewport-responsive table height (v3.8.21)" subsection documenting the height computation. Appendix B Migration 045 entry. Appendix C "Override-capable projection pattern" decision row extended with v3.8.21 footnote naming Multi-Student Discount as the pattern's second instance and codifying the cascade contract for stored overrides on toggleable upstream fields. Needed migrations renumbered 046-051. Document header bumped 3.8.20 → 3.8.21.
- **v3.8.20** — May 4, 2026 — Tuition-CrossModule-KPIs: wire Final Budget and Preliminary Budget KPIs to Tuition data sources via promoted Audit reference snapshot. Final Budget and Preliminary Budget previously had placeholder KPIs ("Pending Tuition Worksheet", "Pending Enrollment Estimator") in the lower half of the `KpiSidebar`. With Tuition operationally complete, this commit wires the five enrollment-dependent KPIs (Number of Students, Current Tuition, Cost per Student, Tuition Gap, Break-even Enrollment) to their actual Tuition data sources. The architectural keystone is the **promoted Audit reference snapshot mechanism**: Final Budget anchors to a specific operator-promoted Audit snapshot rather than live Audit data, separating "what we plan to spend" (Final Budget construction) from "what we are seeing happen" (live Audit). (1) **Migration 043** — `tuition_worksheet_snapshots.is_final_budget_reference bool` + partial unique index enforcing one-promoted-per-scenario invariant. New RPC `mark_snapshot_as_final_budget_reference` atomically demotes any prior reference and promotes the target (uses `set local session_replication_role = replica` to bypass the snapshot-immutability trigger; same pattern as M031/M038 backfills). Writes a synthetic change_log row pointed at the scenario for Recent Activity surfacing per v3.8.17 lesson. (2) **Migration 044** — two read-only KPI reader RPCs: `get_latest_locked_tuition_planning(p_aye_id)` returns the locked Stage 1 snapshot's relevant fields (read by both Preliminary and Final Budget); `get_tuition_audit_final_budget_reference_summary(p_aye_id)` returns the promoted Audit snapshot's aggregates (`total_students`, `total_families`, `net_tuition_for_year`) computed from `tuition_worksheet_snapshot_family_details` per the v3.8.10 round-trip equivalence pattern. Both validate `tuition.view`; both return zero rows when the precondition isn't met so consumers can render "Pending …" KPI subtitles. (3) **`auditLog.js` + `ActivityFeedModal`** extensions: `'snapshot_promoted'` event kind recognized and rendered as "Promoted snapshot to Final Budget reference: \"<label>\"" with gold left rule (matches snapshot capture treatment — both are operator-driven reference-point events). Categorized in governance bucket. (4) **`SnapshotsPanel.jsx`** extensions: per-row "Mark as Final Budget reference" affordance (only on Stage 2 audit snapshots; gated on `tuition.edit`); "Final Budget reference" badge (small gold-bordered pill, font-variant: small-caps) on the currently-promoted row; promote-confirm modal with copy that adapts to whether a prior reference is being replaced. (5) **`KpiSidebar.jsx`** extensions: accepts new props `stageType`, `tuitionStage1`, `tuitionAuditRef`. The bottom half (Number of Students / Current Tuition / Cost per Student / Tuition Gap / Break-even Enrollment) now renders computed values when both the Tuition reference and the live Final Budget total_expense are present; otherwise renders "—" + a "Pending …" subtitle naming the missing upstream. Stage-aware sourcing: Preliminary reads enrollment from Stage 1 `total_students`; Final reads enrollment from the Audit reference's `total_students`. Stage 1 base_rate drives Current Tuition for both. Break-even is the Stage 1 `kpi_breakeven_enrollment` (captured at lock per M032) for Preliminary, but a live recompute (`ceil(totalExpense / (audit_ref.net_tuition_for_year / total_students))`) for Final — sensitive to live Final Budget edits but anchored to the audit's frozen actual-enrollment baseline. Quiet single-line reference note in the sidebar footer naming the snapshot the cross-module KPIs are reading from. (6) **`BudgetStage.jsx`** loads both Tuition refs on AYE change; passes them through to `KpiSidebar`. Tuition refs are CACHED on mount; mid-edit changes to Final Budget recompute KPIs client-side without further round-trips. Re-promoting a different Audit snapshot (rare deliberate action) requires page reload to pick up. (7) **Architecture doc**: §7.3 new "Final Budget reference snapshot (v3.8.20)" subsection codifying the operator-promoted reference mechanism. §7.3 new "Cross-module data flow (v3.8.20)" subsection naming the consumer-side KPI sources per stage with a tabular summary. Appendix B Migrations 043 + 044 entries. Three new Appendix C decision rows: "Final Budget anchors to operator-promoted Audit snapshot, not live Audit data"; "Cross-module KPI consumers compute client-side from cached reads"; "Override-capable projection pattern" (generalized commitment for future cross-module flows where defaults from upstream sources should be operator-overridable). Needed migrations renumbered 045-050. Document header bumped 3.8.19 → 3.8.20. **Out of scope**: Cash Reserve Months wiring (depends on Cash Flow module not yet built); Enrollment Estimator integration (module not built; Stage 1 `total_students` continues as Tuition Committee projection); KPI recomputation triggered by Audit edits AFTER the page is loaded (caching is intentional); drill-down view to inspect data behind a Final Budget KPI; snapshot promotion via Tuition Audit's main page (Snapshots panel is the home for snapshot management).
- **v3.8.19** — May 4, 2026 — Tuition-B2-import polish: validation gap fixes + Notes placeholder removal + spelled-out family/student labels. (1) **Three validation rules from the B2-import spec that didn't engage in M041**: negative discount values now warn instead of erroring (operator decides; value stages as-parsed); unparseable dates flagged client-side now surface in the staging UI as hard errors (Migration 042 has the upload modal send client_parse_errors / client_parse_warnings arrays per row, and the RPC reads them as the starting set, appending server-side outcomes); date_withdrawn before date_enrolled now warns instead of erroring (edge cases are legitimate; operator decides). Migration 042 ships as CREATE OR REPLACE on `tg_validate_tuition_import_row` and `create_tuition_audit_import_batch`; no schema change. The client-side parser (`tuitionImportParser.js`) also gets the spec-aligned date error message and the date-order warning so both layers fire consistently. (2) **Notes placeholder removed**: the `Audit context, FA committee notes, board annotations…` placeholder on the live family table's Notes textarea was visually distracting in the dense table view. Empty Notes cells now render as truly empty in both display and edit states. (3) **"families/students" spelled out**: the Enrolled Families card's Total line previously read `{N} fam · {M} stu`; now reads `{N} families · {M} students` with proper pluralization (`1 family · 1 student` when applicable). Code-comment ASCII layout sketch updated for consistency. **Class-of-issue note**: spec-vs-implementation drift on validation logic is a recurring failure mode — the "happy path" gets implemented; edge-case validators get skipped without surfacing in compile errors or smoke tests. Future builds should explicitly verify each spec'd validation rule fires against intentional bad data before declaring the validation surface complete; an automated test fixture path is queued. Appendix B Migration 042 entry; needed migrations renumbered 043-048. Document header bumped 3.8.18 → 3.8.19.
- **v3.8.18** — May 4, 2026 — Tuition-B2-import: bulk family import via XLSX/CSV with staging review. The "+ Import CSV" placeholder hook from B2-final is wired to a real bulk-import workflow. Operationally significant for schools doing initial start-of-year audits or onboarding; Libertas's AYE 2026 has 84 families, and entering one-by-one is feasible but tedious. The COA bulk-import pattern is the architectural precedent; B2-import follows it with two Tuition-specific refinements: (a) XLSX template with embedded Instructions tab carrying live reference values pulled from the active scenario; (b) per-row computed-column preview in the staging UI for fiscal-decision visibility before commit. (1) **Migration 040 — staging tables**: `tuition_audit_import_batches` (per-upload header with status lifecycle staged/accepted/rejected/expired and append/replace mode chosen at accept time) and `tuition_audit_import_staged_rows` (mirrors family_details schema with all columns nullable; carries parse_errors / parse_warnings jsonb arrays + raw_row jsonb for diagnostic visibility). RLS gated by `tuition.view` + `can_view_family_details`. (2) **Migration 041 — staging RPCs**: three SECURITY DEFINER functions. `create_tuition_audit_import_batch` validates Stage 2 + drafting + caller perms, walks the parsed_rows jsonb array inserting one staged row per element with server-side validation per-row via `tg_validate_tuition_import_row` (defense in depth — client-side validation is for UX only). `accept_tuition_audit_import_batch` validates no row has errors, applies the faculty cascade rules during commit (Migration 022's Stage 2 immutability trigger fires on UPDATE not INSERT, so the locked-from-edit fields populate freely at INSERT time), in replace mode DELETEs existing family_details in the same transaction. `reject_tuition_audit_import_batch` marks the batch rejected and retains staged data for audit. Both accept and reject write a synthetic change_log row pointed at the scenario (`field_name='__import_accepted__'` / `'__import_rejected__'`) for Recent Activity surfacing per v3.8.17 audit-trail-surfacing lesson. (3) **`src/lib/tuitionImportParser.js`** — display column name → snake_case schema mapping; date parsing with format-fallback (MM/DD/YYYY, M/D/YY, ISO, etc.); strict-positive currency parsing with parens/leading-minus warnings; liberal boolean parsing (Yes/Y/TRUE/1; empty = false). (4) **`src/lib/tuitionImportTemplate.js`** — XLSX template generator using SheetJS. Two worksheets: Instructions tab (formatted reference content with step-by-step usage, column descriptions, important rules, and live reference values for AYE/tier_rates/fees/faculty_pct pulled from the active scenario) + Family Data tab (formatted column headers in row 1, otherwise empty). Uses SheetJS CDN tarball (`xlsx-0.20.3.tgz`) installed via `npm install https://cdn.sheetjs.com/xlsx-0.20.3/xlsx-0.20.3.tgz` — patched, zero CVEs vs the npm-published `xlsx` package which has known unfixed vulnerabilities. (5) **`TuitionImportUploadModal.jsx`** — Download Template button + file picker; on file selection parses client-side via SheetJS, normalizes via `tuitionImportParser`, posts to `create_tuition_audit_import_batch` RPC, navigates to the staging review page. (6) **`TuitionAuditImportStaging.jsx`** — staging review page at `/modules/tuition/audit/import/:batchId`. Summary callout (total / errors / warnings / clean counts), mode radio buttons (append default / replace destructive with confirmation modal), staging table, envelope-tracker preview showing post-accept envelope state given the mode toggle, action buttons (Reject with optional reason / Accept; Accept disabled when any row has errors). (7) **`TuitionImportStagingTable.jsx`** — mirrors `TuitionFamilyDetailsTable`'s 13-column layout for visual continuity, plus leading "Row #" column and trailing "Status" column (Clean / N errors / N warnings; clicking expands inline to show detailed messages). Hard errors render with red left rule; warnings with amber. Editing-in-staging is out of scope; operators reject + re-upload to fix. (8) **`auditLog.js` extensions**: `classifyEvent` recognizes `'import_accepted'` and `'import_rejected'` event kinds (via `event.reason.startsWith('import_accepted')` / `'import_rejected'` AND via the `__import_accepted__` / `__import_rejected__` field-name markers as defensive fallback). `summarizeEvent` renders these as "Imported {N} families ({mode} mode) from \"{file_name}\"" / "Rejected import \"{file_name}\". Reason: \"{reason}\"". (9) **`ActivityFeedModal` extensions**: import_accepted gets a green left rule (material data addition); import_rejected gets the muted treatment (declined-without-effect, parallel to unlock_withdrawn). Both are categorized as governance bucket in the filter dropdown. (10) **TuitionAuditPage integration**: "+ Import CSV" button now opens the upload modal (was a placeholder toast in B2-final). (11) **App.jsx**: registers the staging-page route. (12) **`xlsx` (SheetJS) added to package.json**, sourced from the SheetJS CDN tarball — bundle size impact: app.js grew ~520KB (~830KB → ~1354KB) due to SheetJS; expected and acceptable for the operational utility delivered. (13) **Architecture doc**: §7.3 new "Bulk family import (v3.8.18)" subsection codifying the workflow, the parse rules, the append-vs-replace semantics, and the audit-trail surfacing pattern. Appendix B Migrations 040 + 041 entries. Three new Appendix C decision rows: "Tuition import uses XLSX template with embedded Instructions tab; accepts both XLSX and CSV on upload"; "Import mode (append vs replace) chosen at acceptance time, not template time"; "Server-side validation in the import RPC; client-side validation for UX only". Needed migrations renumbered 042-047. Document header bumped 3.8.17 → 3.8.18.
- **v3.8.17** — May 4, 2026 — Tuition-B2-final-fixes. Six refinements surfaced by the B2-final walkthrough, packaged into a single small commit before B2-import: (1) **Snapshot reason taxonomy dropped**: Migration 038's CHECK-constrained taxonomy (`'midyear_reference'` / `'fall_semester_end'` / etc.) is removed via Migration 039. Schools have varying calendars and the taxonomy added rigidity without analytical value. The `snapshot_reason` column is preserved as nullable vestigial for back-compat; future cleanup migration may DROP COLUMN once confirmed unused. (2) **`capture_tuition_audit_snapshot` RPC simplified**: three-arg → two-arg `(p_scenario_id, p_snapshot_label)`. Label is required (RPC raises if null/empty); function body otherwise mirrors M038 with `snapshot_reason` writing NULL. (3) **Snapshot capture audit-log fix**: M038's RPC inserted into `tuition_worksheet_snapshots` which fired `tg_log_changes` and created a `change_log` row, but with `target_table = 'tuition_worksheet_snapshots'` — Tuition-D's `MODULE_AUDIT_CONFIGS.tuition` only queries change_log rows with `target_table = 'tuition_worksheet_scenarios'`, so snapshot events were invisible to Recent Activity. M039's RPC now writes a synthetic change_log row pointed at the scenario row (`field_name = '__snapshot_captured__'`, `new_value = jsonb_build_object('snapshot_id', ..., 'label', ..., 'captured_at', ...)`, `reason = 'snapshot_captured: <label>'`); `auditLog.js` classifyEvent + summarizeEvent gain a `'snapshot_captured'` kind rendered with a gold left rule. Class-of-issue note (per v3.8.12 audit-discipline lesson): RPCs that write to tracked tables must ensure the audit trail surfaces in the user-facing activity feed. (4) **CaptureSnapshotModal simplified**: reason radio buttons removed; modal now shows a single Snapshot Label field (required, with examples in helper text); Confirm button disabled when label is empty. (5) **Discounts card title and Budget column**: card renamed "Discount Envelopes" → "Discounts"; Budget column renders positive amounts ($X,XXX) instead of parens-as-negative — the envelope is a projection, not a realized debit. Used and Remaining columns unchanged (Used in parens; Remaining conditional green/red). (6) **Family table header alignment + weight normalization**: all column headers center-aligned via the `Th` component's default; font weight uniform across headers (the `font-medium` that crept onto the NET Tuition for YEAR header in B2-final removed; body cell emphasis on that column is unchanged). Architecture doc: §7.3 narrative updated to reflect freeform-label model; Appendix B Migration 039 entry; Appendix C revision to the snapshot taxonomy decision row (footnote-style amendment preserving the reasoning trail); needed migrations renumbered 040-045. Document header bumped 3.8.16 → 3.8.17.
- **v3.8.16** — May 4, 2026 — Tuition Audit page redesign + operator-triggered snapshots + architectural correction (Stage 2 is not a lockable artifact). The B2a walkthrough surfaced two issues: (a) the page layout failed as a working tool (no envelope tracker, no enrolled-families summary, no sticky table header, columns in the wrong order), and (b) the planned B2b "Stage 2 lock workflow" was wrong on first principles — the audit is a living working document, not a governance artifact that gets approved and frozen. **B2-final** delivers the redesigned page, replaces the lock-workflow plan with an operator-triggered snapshot capability, and updates the cascade-rules narrative to drop the false "Final Budget requires Stage 2 locked" prerequisite. (1) **Migration 038** — `tuition_worksheet_snapshots` schema: `snapshot_reason text` (CHECK on `'lock' | 'midyear_reference' | 'fall_semester_end' | 'spring_semester_end' | 'school_year_end' | 'other'`), `snapshot_label text`, `captured_at timestamptz NOT NULL DEFAULT now()`. New RPC `capture_tuition_audit_snapshot(p_scenario_id, p_snapshot_reason, p_snapshot_label)` does the operator-triggered capture (validates Stage 2 + drafting, copies all captured-by-value scenario columns plus all family_details rows including M036 additions, returns the new snapshot id, sets `app.change_reason = 'audit_snapshot_captured: <reason>'` for the audit feed). `lock_tuition_scenario` is intentionally NOT modified — Stage 1 has no family_details rows, so the M036 known gap doesn't affect it; the gap is closed for the operator-snapshot path here. (2) **`src/lib/tuitionMath.js`** — three header-zone aggregate helpers: `computeEnvelopesUsed` (4-row envelope tracker with budgets / used / remaining), `computeFamilyDistribution` (counts and percentages by tier_size with 4+ collapse), `computeNetTuitionForYear`. (3) **`src/lib/budgetLock.js`** — comment-only update documenting that Final Budget cascade no longer requires Stage 2 locked (Stage 2 doesn't lock; module-level cascade fires on Stage 1 lock). (4) **`CaptureSnapshotModal.jsx`** — new modal for operator snapshot capture; reason radio buttons + auto-suggested label that's editable. (5) **`SnapshotsPanel.jsx`** — new right-side slide-out panel listing all snapshots chronologically; per-row "View" button is a placeholder for the Tuition-E read-only snapshot view. (6) **`TuitionFamilyDetailsTable.jsx` redesign** — 15-column layout matching the v3 mockup; vertical column dividers at four group boundaries; sticky header; row striping; gold-dot override indicator repositioned from corner-absolute to inline-adjacent (format `● ($X,XXX)`); `# Enr.` integer input with native spinner arrows suppressed; Date Withdrawn null displays as em-dash with click-to-arm-picker affordance (no `mm/dd/yyyy` placeholder). (7) **`TuitionAuditPage.jsx` redesign** — collapsible 3-card reference grid (Tier Rates / Discount Envelopes / Enrolled Families; weighted 1:1.6:1; collapse state persisted to localStorage per scenario); action bar with Sort By dropdown (5 options), + Add Family, + Import CSV (placeholder hook for B2-import; click shows "coming soon" toast); page-header buttons for Capture Snapshot / Snapshots / Recent Activity. Sort applies on dropdown change and first load only — mid-session edits and additions preserve display order; new rows append at end. Stage 2 lock-workflow placeholder removed. (8) **Architecture doc** — §7.3 new "Stage 2 working-document model (v3.8.16)" subsection codifying the corrected model; §7.3 implementation status (Tuition-B2-final) subsection enumerating shipped surfaces; §7.5 cascade table updated (Final Budget no longer requires Stage 2 locked; Stage 2 row clarifies it never locks under v3.8.16 model); Appendix B Migration 038 entry; three new Appendix C decision rows ("Tuition Audit (Stage 2) is a working document, not a lockable artifact"; "Operator-triggered snapshots replace lock workflow for Stage 2"; "Final Budget cascade depends on Tuition Stage 1 lock only"); needed migrations renumbered 039-044. Document header bumped 3.8.15 → 3.8.16. **Out of scope queued for B2-import**: CSV template generation, upload + parse + staging UI, bulk review/accept flow mirroring COA pattern, validation rules for imported family rows, faculty-rule warnings on import.
- **v3.8.15** — May 3, 2026 — Hotfix for Tuition-B2a setup card. The "Begin Tuition Audit" affordance failed at click with "Could not find the function public.create_tuition_scenario_from_snapshot(p_scenario_name, p_source_snapshot_id, p_target_stage_id) in the schema cache." Root cause: parameter naming mismatch between Migration 026 (RPC's third positional arg defined as `p_scenario_label`) and the B2a call site (which passed `p_scenario_name`, the natural English match for "scenario name"). PostgREST resolves named-parameter RPC calls by exact name match — there is no name-fuzzing fallback — so the lookup failed silently in dev until the button was actually clicked in production. Migration 037 drops the original function (parameter names are part of the signature for argument-list match; CREATE OR REPLACE alone cannot rename them) and recreates with `p_scenario_name` matching the B2a call site. Function body preserved verbatim from Migration 029 (most-recent definition prior to this hotfix) — same validation, same field-copy list (M027 + M028 + M029 column additions all included), same SECURITY DEFINER. Bonus: setup card button copy updated from "Set up Audit from Tuition Planning" to "Begin Tuition Audit" per design refinement; the button-internal label and inline code comment both updated for consistency. Lesson reinforces v3.8.12's audit discipline: when calling existing RPCs, verify parameter names match between the function definition and the call site (grep the most-recent migration that defines the function, or run `\df function_name` in the SQL editor, before writing the call). Document header bumped 3.8.14 → 3.8.15.
- **v3.8.14** — May 3, 2026 — Tuition Stage 2 per-family editor delivered (Tuition-B2a). The operational surface where the school records actual enrollment family-by-family, allocates discretionary discount envelopes (Faculty / Other / Financial Aid), and captures audit-trail-grade Notes per family. Mirrors the legacy Google Sheet's column structure (Appendix C v3.8.14) for continuity with the operator's existing mental model. (1) **Migration 036** — three new columns on `tuition_worksheet_family_details` (and the snapshot mirror): `is_faculty_family bool NOT NULL DEFAULT false`, `date_enrolled date`, `date_withdrawn date`. Known gap documented for B2b: `lock_tuition_scenario` snapshot INSERT does not yet capture the three new columns; Stage 2 lock today (only triggerable via direct RPC — no UI affordance until B2b) would lose the data on snapshot capture. (2) **Faculty discount rule codified** — faculty discount REPLACES the multi-student tier discount per Appendix C v3.8.14. Toggling `is_faculty_family` true cascades `applied_tier_size = 1`, `applied_tier_rate = base_rate`, and `faculty_discount_amount = base × students × pct`; Multi-Student Discount displays $0 for faculty families. Manual override supported for non-standard cases (legacy Cookson example) with gold-dot indicator at the cell's top-right. (3) **Six new family-row helpers** in `src/lib/tuitionMath.js`: `computeFamilyAppliedTierRate`, `computeFamilyMultiStudentDiscount`, `computeFamilyFacultyDiscountAuto`, `computeFamilyNetTuition`, `isFamilyFacultyDiscountOverridden`, `isFamilyTierRateOverridden`, plus exported `naturalAppliedTierSize` / `naturalAppliedTierRate` for the save-path cascade. Override detection compares stored vs. auto-computed (Option B storage pattern from M027). (4) **`fetchFamilyNotesHistory(familyId)`** in `src/lib/auditLog.js` — targeted change_log lookup for per-row notes history (filters target_table + target_id + field_name in notes/__insert__/__delete__). Parallels `fetchLineHistory` in spirit; scoped to the notes column on family rows. (5) **`TuitionFamilyDetailsTable.jsx`** — the editor itself. Eleven primary columns + faculty toggle + 2 date columns + row-actions (history clock + delete). Sort: faculty families clustered alphabetically at top with a thin navy-tinted separator rule between groups. Direct-edit-with-undo on save (B1 pattern). Currency cells use the parens-display convention from B1.3. Read-only state preservation: no input chrome when scenario is locked; clock icons hide; Add Family hides; gold dots remain. NO emoji icons anywhere — gold dot (Unicode •) and small navy SVG clock are the only visual chrome. (6) **`TuitionFamilyHistoryModal.jsx`** — per-row notes history modal mirroring Budget's `LineHistoryModal` shape. Notes-field events render with "From: / To:" diff. Locked-state suppression hides the clock affordance. (7) **`TuitionAuditPage.jsx`** — three-state branching: cascade-blocked (no Stage 1 lock), Stage 1 locked but no Stage 2 scenario (renders setup card with "Set up Audit from {Stage 1 label}" button calling `create_tuition_scenario_from_snapshot` RPC), Stage 2 scenario exists (renders the editor). Stage 2 lock workflow placeholder note rendered in the header where the Submit-for-Lock-Review button will eventually live (B2b). (8) **Architecture doc** — §3.3 extended with the AYE / school year / per-family enrollment dates distinction, with forward-pointer to the future Governance Calendar module (Phase 8). §7.3 Stage 2 immutability rules narrative extended with the three new editable fields; new "Faculty discount rule (v3.8.14)" subsection codifying the rule, the override capability, and the storage pattern. New "Implementation status (Tuition-B2a, v3.8.14)" subsection enumerating every shipped surface and the B2b deferral list (Stage 2 lock workflow, envelope running totals, sidebar KPIs, mid-year snapshot). Appendix B Migration 036 entry with the known gap documented for B2b. Three new Appendix C decision rows: "Faculty discount replaces multi-student tier discount (operational rule)", "Stage 2 per-family editor mirrors legacy Google Sheet structure", "School year vs AYE temporal distinction." Needed migrations renumbered 037-042. Document header bumped 3.8.13 → 3.8.14.
- **v3.8.13** — May 3, 2026 — Synthetic test user formally documented as architectural fixture; Tuition module permissions granted. The synthetic user `treasurer.test@libertas.local` was created during Phase 2 Budget testing as the second identity for two-identity unlock workflow testing — one user requests unlock, a second user with `approve_unlock` completes the second approval. The user has held Budget permissions since Phase 2 but had no Tuition permissions, so the v3.8.10 Tuition unlock workflow could not be exercised end-to-end. (1) **Migration 035** grants `approve_unlock` on the Tuition module (subsumes view/edit/submit_lock/approve_lock per the hierarchical enum); also defensively re-grants `approve_unlock` on Budget (the original Phase 2 grant was set via SQL Editor and never captured in a migration — this restores it under version control). Idempotent via `ON CONFLICT DO UPDATE WHERE existing_level < approve_unlock`; never demotes admin. (2) **§9.8 "Synthetic test user fixture"** new subsection documenting the user's purpose, current permission set, the "no edit" intent vs. hierarchical-schema reality (the schema does not support per-permission granularity without subsumption; the testing convention "do not use the synthetic user for data entry" is enforced socially), the maintenance discipline ("every module that ships with a two-identity workflow must grant the synthetic user `approve_unlock` on that module as part of the module's lock-workflow session acceptance criteria"), and the credential-handling note. (3) **Appendix C** new decision row: "Synthetic test user as documented fixture." (4) **§12** cross-cutting checklist note pointing future phases at §9.8. (5) Migration 035 entry in Appendix B; needed migrations renumbered 036-041. Document header bumped 3.8.12 → 3.8.13. Future modules to extend grants for: Staffing, Enrollment Estimator, Tuition Audit (Stage 2 lock workflow when B2 ships), Final Budget cascade work.
- **v3.8.12** — May 3, 2026 — Comprehensive audit and correction of stale column references in tuition RPC bodies. After Migration 033 fixed the read path (`compute_tuition_scenario_kpis`), the first live Approve-and-lock attempt failed on the write path with "column 'other_discount_envelope' of relation 'tuition_worksheet_snapshots' does not exist." Same root cause: Migration 032 had rewritten `lock_tuition_scenario` based on Migration 024's pre-B1.1 baseline rather than the current Migration 029 definition, losing all M027/M028/M029 column updates from the snapshot INSERT. Rather than another whack-a-mole hotfix, Migration 034 audits **every** tuition PL/pgSQL function (8 total: `compute_tuition_scenario_kpis`, `lock_tuition_scenario`, `submit_tuition_scenario_for_lock_review`, `reject_tuition_scenario_lock`, `request_tuition_scenario_unlock`, `approve_tuition_scenario_unlock`, `reject_tuition_scenario_unlock`, `create_tuition_scenario_from_snapshot`) against the current schema. Audit result: only `lock_tuition_scenario` is broken. The other seven are clean — `create_tuition_scenario_from_snapshot` escaped the M032 reset only because M032 didn't rewrite it. Migration 034 ships a single CREATE OR REPLACE for `lock_tuition_scenario` covering every captured-by-value snapshot column organized by schema-evolution era (M023 base + M027/M028/M029/M032 additions); other functions left untouched per don't-rewrite-working-code. **Lesson captured**: future migrations that rewrite a PL/pgSQL function via CREATE OR REPLACE must read the CURRENT function body, not copy from an older migration. The audit habit established here is the safeguard until automated test coverage exists. No schema change; single CREATE OR REPLACE statement; OUT parameter list unchanged. Tuition Stage 1 lock workflow now functions end-to-end with all snapshot columns populated.
- **v3.8.11** — May 3, 2026 — Hotfix for Migration 032's `compute_tuition_scenario_kpis` function body. Migration 027's column renames (`other_discount_envelope` → `projected_other_discount`, `financial_aid_envelope` → `projected_financial_aid`) were not propagated when Migration 032 rewrote the function for Tuition-D KPI extensions; the rewritten body was based on Migration 024's pre-B1.1 definition. Function compiled but raised "record 'v_scenario' has no field 'other_discount_envelope'" at runtime when `lock_tuition_scenario` invoked it. Migration 033 issues a corrected CREATE OR REPLACE that fixes the renames AND restores two other Migration 027 Stage 1 design decisions the Migration 032 rewrite inadvertently reverted: `total_students` read from the column directly (not summed from jsonb), and faculty discount read from `projected_faculty_discount_amount` directly (not computed as `pct × gross`). Both restorations are required for the snapshot's `kpi_breakeven_enrollment` and `kpi_net_education_program_ratio` to match what the sidebar shows within rounding tolerance. Migration 032's intentional Stage 1 B&A revenue projection (`projected_b_a_hours × hourly_rate`) preserved. No schema change; single CREATE OR REPLACE statement; OUT parameter list unchanged from Migration 032. Tuition Stage 1 lock workflow now functions end-to-end. Surfaced during the first live Approve-and-lock attempt against Scenario 1.
- **v3.8.10** — May 3, 2026 — Tuition-D: Stage 1 lock and unlock workflows + Stage 2 setup gateway + cross-module cascade + activity feed + snapshot KPI extension. The largest Tuition session to date — eight surfaces in one commit, predominantly mirroring the patterns Budget already proved (Phase 2 lock workflow, Phase 2 polish unlock workflow, §8.14 setup gateway, §9.1 activity feed). (1) **Migration 032 — snapshot KPI extension + comparator capture**: adds `kpi_breakeven_enrollment int`, `kpi_net_education_program_ratio numeric(8,5)`, `expense_comparator_amount_at_lock numeric(12,2)`, `expense_comparator_source_label_at_lock text` to `tuition_worksheet_snapshots`. Extends `compute_tuition_scenario_kpis` to compute the two new KPIs from the scenario's `expense_comparator_amount` (NULL comparator → both KPIs NULL — Stage 1 may legitimately be locked without a comparator selected). Stage 1 B&A revenue projection now uses `projected_b_a_hours × hourly_rate` (Migration 024 hardcoded 0 for Stage 1 — that simplification is retired). Updates `lock_tuition_scenario` to capture all four new fields into the snapshot. The hybrid KPI computation discipline is preserved: drafting renders client-side via `tuitionMath.js`; lock-time captures server-side for snapshot fidelity. (2) **NEW `src/lib/tuitionWorksheet.js`** — application validator + lock/unlock gating helpers parallel to `budgetLock.js` / `budgetUnlock.js`. Exports `validateScenarioForLock`, `findLockedSibling`, `getLockBannerState`, eight `can*` gates (`canSubmitForLockReview`, `canApproveLock`, `canRejectLock`, `canRequestUnlock`, `canApproveUnlock`, `canRejectUnlock`, `canWithdrawUnlock`), four `validateUnlock*` content-validation wrappers, three RPC wrappers (`submitTuitionScenarioForLockReview`, `approveAndLockTuitionScenario`, `rejectTuitionScenarioLock`), and `TUITION_LOCK_REASON_COPY` for end-user message strings. (3) **Lock UI**: `SubmitForLockReviewModal` (pre-flight modal with admin override path; mirrors Budget's `SubmitLockModal` but without the cascade-rules call since Tuition has no upstream cascade). `LockReviewModal` (approver-facing; one modal hosts both Approve and Reject paths with a reason textarea that becomes required on the Reject path). `TuitionLockBanner` (unified component handling all three lock-state variants — pending, locked-no-request, locked-awaiting-final-approval — replacing Budget's two-component split for tuition's smaller page). (4) **Unlock UI**: `RequestUnlockModal`, `UnlockApprovalModal`, `RejectUnlockModal`, `WithdrawUnlockModal` — each calls its respective Migration 025 RPC; UI shapes mirror Budget's modals with tuition-specific copy ("locked tier rates", "tuition agreements remain in force on unlock", etc.). (5) **Stage 2 setup gateway** (`src/pages/modules/TuitionAuditPage.jsx` + `src/components/tuition/TuitionAuditEmptyState.jsx`): mounted at `/modules/tuition/audit`. Resolves the `final`-typed tuition stage, probes for any locked Stage 1 snapshot in the active AYE, and renders the empty-state with one of two messages (cascade blocked vs. ready-but-editor-pending). The Stage 2 editing surface itself is queued — gateway only for v3.8.10. AppShell sidebar "Audit" child uncommented. App.jsx route registered. (6) **Cross-module cascade** (`src/lib/budgetLock.js`): `getTuitionStage1LockState(ayeId)` helper + `checkCascadeRules` extension. When called with `scenarioStageType: 'preliminary'`, the cascade injects a hardBlock failure when no Tuition Stage 1 snapshot exists for the AYE. Enforced at the application layer only (no DB trigger added in v3.8.10) and prospectively (AYE 2026's Preliminary Budget is grandfathered — see Appendix C "Cross-module cascade enforcement is prospective, not retroactive"). `BudgetStage` passes `scenarioStage` through to `SubmitLockModal`, which forwards `scenarioStageType` to the cascade check. The hardBlock bypass-suppression in SubmitLockModal extended to also count cascade hardBlocks (was: in-memory failures only). (7) **Activity feed generalization** (`src/lib/auditLog.js`): `MODULE_AUDIT_CONFIGS` map keyed by module id; `fetchScenarioActivity` signature changed to `(moduleId, scenarioId, opts)`. Tuition entry maps `scenarioTable: 'tuition_worksheet_scenarios'` and `lineTable: null` (Stage 1 has no per-row line table — configuration lives inline in jsonb columns). `ActivityFeedModal` gains a `moduleId` prop and renders the PDF export link only for budget; tuition shows "PDF export ships in a future commit". The Budget call sites and the Budget Activity print page were updated to pass `'budget'` as the new first arg. Tuition's worksheet page now opens the same modal with `moduleId="tuition"`. (8) **`TuitionWorksheet.jsx` integration**: adds the "Submit for Lock Review" primary button, the lock-state banners, the recommended-scenario inline hint, the sibling-locked drafting banner, the wired Recent Activity link (replacing the disabled stub), and all six new modals (SubmitForLockReview, LockReview, RequestUnlock, UnlockApproval, RejectUnlock, WithdrawUnlock). The Save button stays as the existing confidence-affordance toast. (9) **Architecture doc**: §7.3 gains a new "Implementation status (Tuition-D, v3.8.10)" subsection enumerating every shipped surface plus the Tuition-E deferral list (Tuition Schedule PDF, Stage 2 editing surface, tuition activity print route, year-over-year comparison). §7.5 gains an "Implementation status" paragraph documenting the application-only cross-module cascade enforcement and a "Prospective vs. retroactive" paragraph codifying the grandfathering principle. Appendix B Migration 032 entry; needed migrations renumbered 033-038. Appendix C three new decision rows: "Tuition Stage 1 lock_via is always 'cascade' or 'override'", "Server-side KPI computation at lock time, client-side at drafting time (hybrid)", "Cross-module cascade enforcement is prospective, not retroactive". Document header bumped 3.8.9 → 3.8.10.
