...
**Two-stage modules.** The Budget module's Preliminary / Final stage shape is not unique; the Tuition module follows the same pattern (Tuition Planning + Tuition Audit per architecture §7.3). When a module's stages have asymmetric semantics — Stage 1 sets configuration that downstream commitments rely on; Stage 2 captures actuals against that locked configuration — the pattern is: (1) reuse the `preliminary` and `final` stage types from the existing catalog rather than introducing module-specific types; (2) Stage 2 setup seeds from the locked Stage 1 snapshot via a `create_<module>_<stage2>_from_<stage1>_snapshot` RPC parallel to Migration 019's `create_scenario_from_snapshot` for Final Budget; (3) Stage 2 immutability — fields configured at Stage 1 lock that downstream parties (families, contracted staff, etc.) have already committed against — is enforced via a BEFORE UPDATE trigger using the same three-layer pattern as sibling-lock guards. School display names carry the module-specific vocabulary; the stage type drives generic machinery.

**Cross-module lock cascade rules.** Each module's section in the architecture doc points to §7.5 for the canonical cascade definition rather than restating the cascade locally. Cascade rules live in `school_lock_cascade_rules` and gate submit-for-lock validation, not draft editing. Override paths exist for every cascade rule — the realistic day-one path while upstream modules have not yet shipped is override with justification. Per-stage cascade variations (e.g., "Final Budget specifically requires locked Staffing while Preliminary is fine with projected") are handled by the per-stage Tuition / Budget framing today; if richer per-stage rule rows become a real need, the schema extension is described in architecture §3.4 "Granularity."

**Sidebar structure for staged modules.** Modules whose work is split across stages render in the sidebar as collapsible parents with stage children, not as flat items. Today: Tuition has children (Planning today; Audit when Phase 4 ships Stage 2) and Budget has children (Preliminary, Final). The collapsible-parent pattern matches the section-header collapse behavior at one level of nesting deeper: chevron rotates ▾/▸; parent click toggles expand/collapse only and does not navigate; navigation happens via clicking a child. Single-stage modules in the same section (Enrollment Estimator, Staffing) remain top-level until they grow stages of their own. Parent items use `toggleOnly: true` in the `NAV_SECTIONS` config to opt into this pattern; existing navigable parents (e.g. School Settings under Admin) keep the legacy "navigable parent + subItems" pattern by leaving `toggleOnly` unset. The sidebar deliberately uses no numbers — temporal sequence is the governance calendar's job, not the sidebar's. Stage child labels for the Budget parent are derived sidebar-side by stripping the trailing " Budget" off `display_name` so the parent carries the module name and the children carry the stage names ("Preliminary" / "Final"); other UI surfaces continue to read `display_name` / `short_name` as canonical. See architecture §3.2.
